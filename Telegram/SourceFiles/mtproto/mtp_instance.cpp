...
namespace MTP
{
	...
	void Instance::Private::start(Config&& config)
	{
		...
		_checkDelayedTimer.setCallback([this]
		{
	...
	void Instance::Private::requestConfig()
	{
		...
		_configLoader = std::make_unique<internal::ConfigLoader>(
			_instance,
			_userPhone,
			rpcDone([=](const MTPConfig& result)
			{
	...
	void Instance::Private::stopSession(ShiftedDcId shiftedDcId)
	{
		...
		if (it != _sessions.end())
		{
			if (it->second.get() != _mainSession)
			{
				// don't stop main session
				it->second->stop();
			}
		}
	}
	...
	bool Instance::Private::rpcErrorOccured(mtpRequestId requestId, const RPCFailHandlerPtr& onFail, const RPCError& err)
	{
		// return true if need to clean request data
	...
	void Instance::Private::performKeyDestroy(ShiftedDcId shiftedDcId)
	{
		...
		_instance->send(MTPDestroy_auth_key(), rpcDone([this, shiftedDcId](const MTPDestroyAuthKeyRes& result)
		{
			switch (result.type())
			{
			case mtpc_destroy_auth_key_ok: LOG(("MTP Info: key %1 destroyed.").arg(shiftedDcId));
				break;
			case mtpc_destroy_auth_key_fail:
				...
				break;
			case mtpc_destroy_auth_key_none: LOG(("MTP Info: key %1 already destroyed.").arg(shiftedDcId));
				break;
			}
	...
	int32 Instance::state(mtpRequestId requestId)
	{
		// < 0 means waiting for such count of ms
