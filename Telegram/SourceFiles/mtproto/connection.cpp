	...
	namespace internal
	{
		namespace
		{
			...
			void wrapInvokeAfter(SecureRequest& to, const SecureRequest& from, const RequestMap& haveSent, int32 skipBeforeRequest = 0)
			{
				...
				if (i == haveSent.constEnd())
				{
					// no invoke after or such msg was not sent or was completed recently
		...
		ConnectionPrivate::ConnectionPrivate(
			...
			, _oldConnectionTimer(thread, [=]
			{
				markConnectionOld();
			})
			, _waitForConnectedTimer(thread, [=]
			{
				waitConnectedFailed();
			})
			, _waitForReceivedTimer(thread, [=]
			{
				waitReceivedFailed();
			})
			, _waitForBetterTimer(thread, [=]
			{
				waitBetterFailed();
			})
		...
		{
			// recreate all msg_id and msg_seqno
			...
			for (auto i = setSeqNumbers.cbegin(), e = setSeqNumbers.cend(); i != e; ++i)
			{
				// generate new seq_numbers
				bool wasNeedAck = (*(i.value()->data() + 6) & 1);
				*(i.value()->data() + 6) = sessionData->nextRequestSeqNumber(wasNeedAck);
			}
			if (!replaces.isEmpty())
			{
				for (auto i = replaces.cbegin(), e = replaces.cend(); i != e; ++i)
				{
					// replace msgIds keys in all data structs
				...
				for (auto i = haveSent.cbegin(), e = haveSent.cend(); i != e; ++i)
				{
					// replace msgIds in saved containers
					if (i.value().isSentContainer())
					{
		...
		void ConnectionPrivate::tryToSend()
		{
			...
			if (_pingIdToSend)
			{
				...
				if (_shiftedDcId == BareDcId(_shiftedDcId) && !prependOnly)
				{
					// main session
		...
		const auto appVersion = QString::fromLatin1(AppVersionStr)
			...
			SecureRequest toSendRequest;
			{
				...
				if (toSendCount == 1 && first->msDate > 0)
				{
					// if can send without container
					toSendRequest = first;
					if (!prependOnly)
					{
						toSend.clear();
				...
				case mtpc_bad_msg_notification:
					{
							...
							|| errorCode == 64)
						{
							...
							{
								// must create new session, because msg_id and msg_seqno are inconsistent
								if (badTime)
								{
									if (serverSalt) sessionData->setSalt(serverSalt);
									unixtimeSet(serverTime, true);
						...
						{
							// fatal (except 48, but it must not get here)
							...
							if (requestId)
							{
								...
								_instance->clearCallbacksDelayed({
									1, RPCCallbackClear(
										requestId,
										-errorCode)
								});
							...
							for (uint32 i = 0, l = idsCount; i < l; ++i)
							{
								...
								else
								{
									...
									else
									{
										...
										if (msgIdState == ReceivedMsgIds::State::NeedsAck)
										{
											// need ack, so we sent ack
											state |= 0x08;
										}
										else
										{
				...
				case mtpc_msgs_state_info:
					{
						...
						SecureRequest requestBuffer;
						{
							...
							if (replyTo == haveSent.cend())
							{
								// do not look in toResend, because we do not resend msgs_state_req requests
		...
		bool ConnectionPrivate::requestsFixTimeSalt(const QVector<MTPlong>& ids, int32 serverTime, uint64 serverSalt)
		{
			...
			for (uint32 i = 0; i < idsCount; ++i)
			{
				if (wasSent(ids[i].v))
				{
					// found such msg_id in recent acked requests or in recent sent requests
					if (serverSalt) sessionData->setSalt(serverSalt);
					unixtimeSet(serverTime, true);
					return true;
				}
					...
					for (uint32 i = 0; i < idsCount; ++i)
					{
						...
						if (req != haveSent.cend())
						{
							...
							else
							{
								...
								if (!moveToAcked)
								{
									// ignore ACK, if we need a response (if we have a handler)
