	...
	})
{
	...
	subscribe(session().calls().currentCallChanged(), [this](Calls::Call* call)
	{
		setCurrentCall(call);
	});
...
void MainWidget::createPlayer()
{
	if (!_player)
	{
		...
		rpl::merge(
			...
			_player->lifetime());
		_player->entity()->setCloseCallback([this]
		{
			closeBothPlayers();
		});
...
void MainWidget::createExportTopBar(Export::View::Content&& data)
{
	...
	rpl::merge(
		_exportTopBar->heightValue() | rpl::map([]
		{
			return true;
		}),
...
void MainWidget::updateOnline(bool gotOtherOffline)
{
		...
		|| (isOnline && gotOtherOffline))
	{
		...
		if (!isOnline)
		{
			// Went offline, so we need to save message draft to the cloud.
			saveDraftToCloud();
		}
	...
	case mtpc_updateNewChannelMessage:
		{
			...
			{
				if (channel->ptsRequesting())
				{
					// skip global updates while getting channel difference
					return;
				}
	...
	case mtpc_updateUserStatus:
		{
			...
			if (auto user = session().data().userLoaded(d.vuser_id.v))
			{
				...
				case mtpc_userStatusOffline:
					user->onlineTill = d.vstatus.c_userStatusOffline().vwas_online.v;
					break;
