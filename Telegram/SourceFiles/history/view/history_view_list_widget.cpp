...
void ListWidget::refreshRows(const Data::MessagesSlice &old) {
	...
	// With a screen reader an arrival is read only once the focus reaches
	// it: being at the end advances nothing, and the bar is created above
	// the new messages instead of consumed.
	const auto readingAtEnd = scrolledTillEnd
		&& !Ui::ScreenReaderModeActive();
	if (readingAtEnd && addedToEndCount > 0) {
		_delegate->listItemsAddedToEnd(_items, addedToEndCount);
	}
	const auto markLastAsRead = (readingAtEnd && markingMessagesRead());
