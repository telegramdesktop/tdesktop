...
QPixmap Sticker::paintedPixmap(const PaintContext &context) const {
	...
	const auto adjust = [&](int side) {
		const auto scaled = int(base::SafeRound(side * ratio));
		return (((scaled / 8) * 8) / ratio);
	};
	...
	if (_data->sticker()->isLottie()) {
		_player = std::make_unique<LottiePlayer>(
			ChatHelpers::LottiePlayerFromDocument(
				...
				_cachingTag,
				QSize(
					int(base::SafeRound(countOptimalSize().width()
						* style::DevicePixelRatio())),
					int(base::SafeRound(countOptimalSize().height()
						* style::DevicePixelRatio()))),
				Lottie::Quality::High));
