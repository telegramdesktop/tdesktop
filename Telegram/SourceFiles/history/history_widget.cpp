	...
	, _topShadow(this)
{
	...
	_historyDown->addClickHandler([=]
	{
		historyDownClicked();
	});
	_unreadMentions->addClickHandler([=]
	{
	...
	_highlightTimer.setCallback([this]
	{
	...
	_send->setRecordStartCallback([this]
	{
		recordStartCallback();
	});
	_send->setRecordStopCallback([this](bool active)
	{
		recordStopCallback(active);
	});
	_send->setRecordUpdateCallback([this](QPoint globalPos)
	{
		recordUpdateCallback(globalPos);
	});
	_send->setRecordAnimationCallback([this]
	{
		updateField();
	});
	...
	int32 progress)
{
	...
	if (history->mySendActionUpdated(type, doing))
	{
		...
		if (doing)
		{
			...
			{
			case Type::Typing:
				action = MTP_sendMessageTypingAction();
				break;
			case Type::RecordVideo:
				action = MTP_sendMessageRecordVideoAction();
				break;
			case Type::UploadVideo:
				action = MTP_sendMessageUploadVideoAction(MTP_int(progress));
				break;
			case Type::RecordVoice:
				action = MTP_sendMessageRecordAudioAction();
				break;
			case Type::UploadVoice:
				action = MTP_sendMessageUploadAudioAction(MTP_int(progress));
				break;
			case Type::RecordRound:
				action = MTP_sendMessageRecordRoundAction();
				break;
			case Type::UploadRound:
				action = MTP_sendMessageUploadRoundAction(MTP_int(progress));
				break;
			case Type::UploadPhoto:
				action = MTP_sendMessageUploadPhotoAction(MTP_int(progress));
				break;
			case Type::UploadFile:
				action = MTP_sendMessageUploadDocumentAction(MTP_int(progress));
				break;
			case Type::ChooseLocation:
				action = MTP_sendMessageGeoLocationAction();
				break;
...
void HistoryWidget::updateReportSpamStatus()
{
	...
	else
	{
		...
		else if (_peer->migrateFrom())
		{
			// migrate report status
...
void HistoryWidget::sendBotCommand(PeerData* peer, UserData* bot, const QString& cmd, MsgId replyTo)
{
	// replyTo != 0 from ReplyKeyboardMarkup, == 0 from cmd links
	...
	const Storage::PreparedList& list) const
{
	const auto text = [&]
	{
		...
		case Error::NonLocalUrl:
			return lng_send_image_empty(
				lt_name,
				list.errorData);
		case Error::TooLargeFile:
			return lng_send_image_too_large(
				lt_name,
				list.errorData);
		}
		return lang(lng_forward_send_files_cant);
...
void HistoryWidget::updateHistoryDownVisibility()
{
	...
	if (_historyDownIsShown != historyDownIsShown)
	{
		...
		_historyDownShown.start([=]
		{
			updateHistoryDownPosition();
		}, _historyDownIsShown ? 0. : 1., _historyDownIsShown ? 1. : 0., st::historyToDownDuration);
	}
}
...
void HistoryWidget::updateUnreadMentionsVisibility()
{
	...
	if (_unreadMentionsIsShown != unreadMentionsIsShown)
	{
		...
		_unreadMentionsShown.start([=]
		{
			updateUnreadMentionsPosition();
		}, _unreadMentionsIsShown ? 0. : 1., _unreadMentionsIsShown ? 1. : 0., st::historyToDownDuration);
	}
}
