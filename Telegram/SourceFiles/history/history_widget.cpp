...
namespace
{
	...
	constexpr auto kCommonModifiers = 0 | Qt::ShiftModifier | Qt::MetaModifier | Qt::ControlModifier;
	const auto kPsaAboutPrefix = "cloud_lng_about_psa_";

	[[nodiscard]] rpl::producer<PeerData *> ActivePeerValue(
		not_null<Window::SessionController *> controller)
	{
		return controller->activeChatValue() | rpl::map([](Dialogs::Key key)
														{
...
HistoryWidget::HistoryWidget(
	...
	: Window::AbstractSectionWidget(
	  ...
	  _api(&controller->session().mtp()), _updateEditTimeLeftDisplay([=]
																	 { updateField(); }),
	  _fieldBarCancel(this, st::historyReplyCancel), _topBars(std::make_unique<Ui::RpWidget>(this)), _topBar(this, controller), _scroll(
																																	this,
																																	controller->chatStyle()->value(lifetime(), st::historyScroll),
																																	false),
	  _updateHistoryItems([=]
						  { updateHistoryItemsByTimer(); }),
	  _cornerButtons(
		  _scroll.data(),
	  ...
	  _supportAutocomplete(session().supportMode()
							   ...
							   : nullptr),
	  _send(std::make_shared<Ui::SendButton>(this, st::historySend)), _unblock(
																		  this,
	  ...
	  _field(
		  ...
		  tr::lng_message_ph()),
	  _kbScroll(this, st::botKbScroll), _keyboard(_kbScroll->setOwnedWidget(object_ptr<BotKeyboard>(
											controller,
											this))),
	  _membersDropdownShowTimer([=]
								{ showMembersDropdown(); }),
	  _highlighter(
		  &session().data(),
	  ...
	  _topShadow(this)
{
	...
	_scroll->scrolls() | rpl::start_with_next([=]
											  { handleScroll(); }, lifetime());
	_scroll->geometryChanged() | rpl::start_with_next(crl::guard(_list, [=]
	...
	_fieldBarCancel->addClickHandler([=]
									 { cancelFieldAreaState(); });
	_send->addClickHandler([=]
						   { sendButtonClicked(); });

	_mediaEditManager.updateRequests() | rpl::start_with_next([this]
															  { updateOverStates(mapFromGlobal(QCursor::pos())); }, lifetime());

	{
		using namespace SendMenu;
		const auto sendAction = [=](Action action, Details)
		{
			if (action.type == ActionType::CaptionUp || action.type == ActionType::CaptionDown || action.type == ActionType::SpoilerOn || action.type == ActionType::SpoilerOff)
			{
	...
	_unblock->addClickHandler([=]
							  { unblockUser(); });
	_botStart->addClickHandler([=]
							   { sendBotStartCommand(); });
	...
	_reportMessages->addClickHandler([=]
									 { reportSelectedMessages(); });
	_field->submits() | rpl::start_with_next([=](Qt::KeyboardModifiers modifiers)
											 { sendWithModifiers(modifiers); }, _field->lifetime());
	_field->cancelled() | rpl::start_with_next([=]
											   { escape(); }, _field->lifetime());
	_field->tabbed() | rpl::start_with_next([=]
											{ fieldTabbed(); }, _field->lifetime());
	_field->heightChanges() | rpl::start_with_next([=]
												   { fieldResized(); }, _field->lifetime());
	_field->focusedChanges() | rpl::filter(rpl::mappers::_1) | rpl::start_with_next([=]
																					{ fieldFocused(); }, _field->lifetime());
	_field->changes() | rpl::start_with_next([=]
											 { fieldChanged(); }, _field->lifetime());
#ifdef Q_OS_MAC
	// Removed an ability to insert text from the menu bar
	// when the field is hidden.
	_field->shownValue() | rpl::start_with_next([=](bool shown)
												{ _field->setEnabled(shown); }, _field->lifetime());
#endif // Q_OS_MAC
	controller->widget()->shownValue() | rpl::skip(1) | rpl::start_with_next([=]
																			 { windowIsVisibleChanged(); }, lifetime());
	...
	rpl::merge(
		...
		rpl::start_with_next([=]
							 { saveDraftDelayed(); }, _field->lifetime());
	...
	controller->chatStyle()->paletteChanged() | rpl::start_with_next([=]
																	 { _scroll->updateBars(); }, lifetime());

	_forwardPanel->itemsUpdated() | rpl::start_with_next([=]
														 {
		updateControlsVisibility();
		updateControlsGeometry(); }, lifetime());

	InitMessageField(controller, _field, [=](not_null<DocumentData *> document)
					 {
	...
	_fieldCharsCountManager.limitExceeds() | rpl::start_with_next([=]
																  {
		...
		updateFieldSize();
		moveFieldControls(); }, lifetime());

	_send->widthValue() | rpl::skip(1) | rpl::start_with_next([=]
															  {
		updateFieldSize();
		moveFieldControls(); }, _send->lifetime());

	_keyboard->sendCommandRequests() | rpl::start_with_next([=](Bot::SendCommandRequest r)
															{ sendBotCommand(r); }, lifetime());
	...
	rpl::merge(
		...
		session().changes().peerUpdates(
			Data::PeerUpdate::Flag::Rights | Data::PeerUpdate::Flag::StarsPerMessage) |
			rpl::filter([=](const Data::PeerUpdate &update)
						{ return update.peer == _peer; }) |
			rpl::to_empty) |
		rpl::start_with_next([=]
							 { refreshAttachBotsMenu(); }, lifetime());

	_botKeyboardShow->addClickHandler([=]
									  { toggleKeyboard(); });
	_botKeyboardHide->addClickHandler([=]
									  { toggleKeyboard(); });
	_botCommandStart->addClickHandler([=]
									  { startBotCommand(); });
	...
	_attachDragAreas = DragArea::SetupDragAreaToContainer(
		...
		crl::guard(this, [=](bool f)
				   { _field->setAcceptDrops(f); }),
		crl::guard(this, [=]
	...
	Core::App().mediaDevices().recordAvailabilityValue() | rpl::start_with_next([=](Webrtc::RecordAvailability value)
																				{
	...
	session().data().newItemAdded() | rpl::start_with_next([=](not_null<HistoryItem *> item)
														   { newItemAdded(item); }, lifetime());

	session().data().historyChanged() | rpl::start_with_next([=](not_null<History *> history)
															 { handleHistoryChange(history); }, lifetime());

	session().data().viewResizeRequest() | rpl::start_with_next([=](not_null<HistoryView::Element *> view)
																{
	...
	session().data().itemDataChanges() | rpl::filter([=](not_null<HistoryItem *> item)
													 { return !_list && (item->mainView() != nullptr); }) |
		rpl::start_with_next([=](not_null<HistoryItem *> item)
							 { item->mainView()->itemDataChanged(); }, lifetime());

	Core::App().settings().largeEmojiChanges() | rpl::start_with_next([=]
																	  { crl::on_main(this, [=]
																					 { updateHistoryGeometry(); }); }, lifetime());
	Core::App().settings().sendSubmitWayValue() | rpl::start_with_next([=]
																	   { crl::on_main(this, [=]
																					  { updateFieldSubmitSettings(); }); }, lifetime());

	session().data().channelDifferenceTooLong() | rpl::filter([=](not_null<ChannelData *> channel)
															  { return _peer == channel.get(); }) |
		rpl::start_with_next([=]
							 {
		_cornerButtons.updateJumpDownVisibility();
		preloadHistoryIfNeeded(); }, lifetime());

	session().data().userIsBotChanges() | rpl::filter([=](not_null<UserData *> user)
	...
	session().data().botCommandsChanges() | rpl::filter([=](not_null<PeerData *> peer)
	...
	session().changes().entryUpdates(
		EntryUpdateFlag::HasPinnedMessages | EntryUpdateFlag::ForwardDraft) |
	...
	session().changes().historyUpdates(
		HistoryUpdateFlag::MessageSent | HistoryUpdateFlag::BotKeyboard | HistoryUpdateFlag::CloudDraft | HistoryUpdateFlag::UnreadMentions | HistoryUpdateFlag::UnreadReactions | HistoryUpdateFlag::UnreadView | HistoryUpdateFlag::TopPromoted | HistoryUpdateFlag::ClientSideMessages) |
	...
	session().changes().messageUpdates(
		MessageUpdateFlag::Destroyed | MessageUpdateFlag::Edited | MessageUpdateFlag::ReplyMarkup | MessageUpdateFlag::BotCallbackSent) |
	...
	session().changes().realtimeMessageUpdates(
		...
		rpl::start_with_next([=](const Data::MessageUpdate &update)
							 { maybeMarkReactionsRead(update.item); }, lifetime());

	session().data().sentToScheduled() | rpl::start_with_next([=](const Data::SentToScheduled &value)
															  {
	...
	session().data().sentFromScheduled() | rpl::start_with_next([=](const Data::SentFromScheduled &value)
																{
	...
	Media::Player::instance()->switchToNextEvents() | rpl::filter([=](const MediaSwitch &pair)
																  { return (pair.from.type() == AudioMsgId::Type::Voice); }) |
		rpl::start_with_next([=](const MediaSwitch &pair)
							 { scrollToCurrentVoiceMessage(pair.from.contextId(), pair.to); }, lifetime());

	session().user()->flagsValue() | rpl::start_with_next([=](UserData::Flags::Change change)
														  {
	...
	session().changes().peerUpdates(
		PeerUpdateFlag::Rights | PeerUpdateFlag::Migration | PeerUpdateFlag::UnavailableReason | PeerUpdateFlag::IsBlocked | PeerUpdateFlag::Admins | PeerUpdateFlag::Members | PeerUpdateFlag::OnlineStatus | PeerUpdateFlag::Notifications | PeerUpdateFlag::ChannelAmIn | PeerUpdateFlag::DiscussionLink | PeerUpdateFlag::Slowmode | PeerUpdateFlag::BotStartToken | PeerUpdateFlag::MessagesTTL | PeerUpdateFlag::ChatThemeToken | PeerUpdateFlag::FullInfo | PeerUpdateFlag::StarsPerMessage | PeerUpdateFlag::GiftSettings) |
		rpl::filter([=](const Data::PeerUpdate &update)
					{
	...
	rpl::merge(
		...
		rpl::start_with_next([=]
							 { updateNotifyControls(); }, lifetime());

	session().data().itemVisibilityQueries() | rpl::filter([=](const Data::Session::ItemVisibilityQuery &query)
														   { return !_showAnimation && (_history == query.item->history()) && (query.item->mainView() != nullptr) && isVisible(); }) |
		rpl::start_with_next([=](
	...
	_topBar->membersShowAreaActive() | rpl::start_with_next([=](bool active)
															{ setMembersShowAreaActive(active); }, _topBar->lifetime());
	_topBar->forwardSelectionRequest() | rpl::start_with_next([=]
															  { forwardSelected(); }, _topBar->lifetime());
	_topBar->deleteSelectionRequest() | rpl::start_with_next([=]
															 { confirmDeleteSelected(); }, _topBar->lifetime());
	_topBar->clearSelectionRequest() | rpl::start_with_next([=]
															{ clearSelected(); }, _topBar->lifetime());
	_topBar->cancelChooseForReportRequest() | rpl::start_with_next([=]
																   { setChooseReportMessagesDetails({}, nullptr); }, _topBar->lifetime());
	_topBar->searchRequest() | rpl::start_with_next([=]
													{
		if (_history) {
			controller->searchInChat(_history);
		} }, _topBar->lifetime());

	session().api().sendActions() | rpl::filter([=](const Api::SendAction &action)
	...
	{
		session().data().chatListEntryRefreshes() | rpl::start_with_next([=]
																		 { crl::on_main(this, [=]
																						{ checkSupportPreload(true); }); }, lifetime());
	}

	Core::App().materializeLocalDraftsRequests() | rpl::start_with_next([=]
																		{ saveFieldToHistoryLocalDraft(); }, lifetime());
...
void HistoryWidget::refreshGiftToChannelShown()
{
	...
	_giftToChannel->setVisible(channel && channel->isBroadcast() && channel->stargiftsAvailable());
}
...
void HistoryWidget::refreshDirectMessageShown()
{
	...
	if (visible)
	{
		...
		_directMessageLifetime = monoforum->flagsValue() | rpl::skip(1) | rpl::start_with_next([=](Flags::Change change)
																							   {
...
void HistoryWidget::initVoiceRecordBar()
{
	...
	_voiceRecordBar->sendActionUpdates() | rpl::start_with_next([=](const auto &data)
																{
		if (!_history) {
			return;
		}
		session().sendProgressManager().update(
			_history,
			data.type,
			data.progress); }, lifetime());

	_voiceRecordBar->sendVoiceRequests() | rpl::start_with_next([=](const VoiceToSend &data)
																{ sendVoice(data); }, lifetime());

	_voiceRecordBar->cancelRequests() | rpl::start_with_next(applyLocalDraft, lifetime());

	_voiceRecordBar->lockShowStarts() | rpl::start_with_next([=]
															 {
		_cornerButtons.updateJumpDownVisibility();
		_cornerButtons.updateUnreadThingsVisibility(); }, lifetime());

	_voiceRecordBar->errors() | rpl::start_with_next([=](::Media::Capture::Error error)
													 {
	...
	_voiceRecordBar->updateSendButtonTypeRequests() | rpl::start_with_next([=]
																		   { updateSendButtonType(); }, lifetime());

	_voiceRecordBar->lockViewportEvents() | rpl::start_with_next([=](not_null<QEvent *> e)
																 { _scroll->viewportEvent(e); }, lifetime());

	_voiceRecordBar->recordingTipRequests() | rpl::start_with_next([=]
																   {
	...
	_voiceRecordBar->recordingStateChanges() | rpl::start_with_next([=](bool active)
																	{ controller()->widget()->setInnerFocus(); }, lifetime());
...
void HistoryWidget::initTabbedSelector()
{
	...
	selector->emojiChosen() | rpl::filter([=]
										  { return !isHidden() && !_field->isHidden(); }) |
		rpl::start_with_next([=](ChatHelpers::EmojiChosen data)
							 { Ui::InsertEmojiAtCursor(_field->textCursor(), data.emoji); }, lifetime());
	...
	rpl::merge(
		...
		rpl::start_with_next([=](ChatHelpers::FileChosen &&data)
							 { fileChosen(std::move(data)); }, lifetime());

	selector->photoChosen() | filter | rpl::start_with_next([=](ChatHelpers::PhotoChosen data)
															{ sendExistingPhoto(data.photo, data.options); }, lifetime());

	selector->inlineResultChosen() | filter | rpl::filter([=](const ChatHelpers::InlineChosen &data)
														  {
		...
		rpl::start_with_next([=](ChatHelpers::InlineChosen data)
							 { sendInlineResult(data); }, lifetime());

	selector->contextMenuRequested() | filter | rpl::start_with_next([=]
																	 { selector->showMenuWithDetails(sendMenuDetails()); }, lifetime());

	selector->choosingStickerUpdated() | rpl::start_with_next([=](const Selector::Action &data)
															  {
		if (!_history) {
			return;
		}
...
void HistoryWidget::supportInitAutocomplete()
{
	...
	_supportAutocomplete->insertRequests() | rpl::start_with_next([=](const QString &text)
																  { supportInsertText(text); }, _supportAutocomplete->lifetime());

	_supportAutocomplete->shareContactRequests() | rpl::start_with_next([=](const Support::Contact &contact)
																		{ supportShareContact(contact); }, _supportAutocomplete->lifetime());
}
	...
	not_null<HistoryView::Element *> view) const
{
	...
	else if (const auto highlight = itemHighlight(item); (!highlight.range.empty() || highlight.todoItemId) && !IsSubGroupSelection(highlight.range))
	{
		...
		const auto end = sel.empty()
							 ? (todoy + 4 * single)
							 : (HistoryView::FindViewY(view, sel.to, begin + single) + 2 * single);
		auto result = itemTop;
	...
	else if (reactionCenter >= 0)
	{
		...
		const auto byReactionResult = itemTop + reactionCenter - visibleAreaHeight / 2;
			...
			if (_history) {
				session().sendProgressManager().update(
					...
					Api::SendProgressType::ChooseSticker);
														  ...
														  .setText = [=](TextWithTags text)
														  { if (_peer) setFieldText(text); },
	...
	if (bot)
	{
		...
		if (!_inlineResults)
		{
			...
			_inlineResults->setSendMenuDetails([=]
											   { return sendMenuDetails(); });
			_inlineResults->requesting() | rpl::start_with_next([=](bool requesting)
																{ _tabbedSelectorToggle->setLoading(requesting); }, _inlineResults->lifetime());
			updateControlsGeometry();
			orderWidgets();
		}
	...
	std::optional<bool> show)
{
	...
	_chooseTheme = std::make_unique<Ui::ChooseThemeController>(
		...
		controller(),
		peer);
	_chooseTheme->shouldBeShownValue() | rpl::start_with_next(update, _chooseTheme->lifetime());
}
...
void HistoryWidget::saveDraftDelayed()
{
	...
	if (!_field->textCursor().position() && !_field->textCursor().anchor() && !_field->scrollTop().current())
	{
...
void HistoryWidget::fileChosen(ChatHelpers::FileChosen &&data)
{
	...
	if (const auto info = data.document->sticker(); info && info->setType == Data::StickersType::Emoji)
	{
		if (data.document->isPremiumEmoji() && !session().premium() && (!_peer || !Data::AllowEmojiWithoutPremium(_peer, data.document)))
		{
...
void HistoryWidget::setInnerFocus()
{
	if (_list)
	{
		...
		else if (_showAnimation || _nonEmptySelection || (_list && _list->wasSelectedText()) || isRecording() || isJoinChannel() || isBotStart() || isBlocked() || (!_canSendTexts && !_editMsgId))
		{
			if (_scroll->isHidden())
			{
				setFocus();
...
void HistoryWidget::tryProcessKeyInput(not_null<QKeyEvent *> e)
{
	e->accept();
	keyPressEvent(e);
	if (!e->isAccepted() && _canSendTexts && _field->isVisible() && !e->text().isEmpty())
	{
		_field->setFocusFast();
...
void HistoryWidget::setupShortcuts()
{
	Shortcuts::Requests() | rpl::filter([=]
										{ return _history && Ui::AppInFocus() && Ui::InFocusChain(this) && !controller()->isLayerShown() && window()->isActiveWindow(); }) |
...
void HistoryWidget::setupGiftToChannelButton()
{
	...
	widthValue() | rpl::start_with_next([=](int width)
										{ _giftToChannel->moveToRight(0, 0, width); }, _giftToChannel->lifetime());
...
void HistoryWidget::setupDirectMessageButton()
{
	...
	widthValue() | rpl::start_with_next([=](int width)
										{ _directMessage->moveToLeft(0, 0, width); }, _directMessage->lifetime());
...
bool HistoryWidget::applyDraft(FieldHistoryAction fieldHistoryAction)
{
	...
	{
		const auto fieldWillBeHiddenAfterEdit = (!fieldAvailable && _editMsgId != 0);
		clearFieldText(0, fieldHistoryAction);
		setInnerFocus();
	...
	if (_preview)
	{
		_preview->setDisabled(_editMsgId && _replyEditMsg && _replyEditMsg->media() && !_replyEditMsg->media()->webpage());
		if (!_editMsgId)
		{
			_preview->apply(draft->webpage, true);
		}
		else if (!_replyEditMsg || !_replyEditMsg->media() || _replyEditMsg->media()->webpage())
		{
	...
	const Window::SectionShow &params)
{
	...
	if (_history)
	{
		if (_peer->id == peerId)
		{
			...
			else if (showAtMsgId == ShowAtUnreadMsgId && insideJumpToEndInsteadOfToUnread())
			{
				DEBUG_LOG(("JumpToEnd(%1, %2, %3): "
			...
			if (!IsServerMsgId(showAtMsgId) && !IsClientMsgId(showAtMsgId) && !IsServerMsgId(-showAtMsgId))
			{
				// To end or to unread.
				destroyUnreadBar();
			}
			...
			if (!canShowNow)
			{
				...
				{
					DEBUG_LOG(("JumpToEnd(%1, %2, %3): Showing delayed at %4.").arg(_history->peer->name()).arg(_history->inboxReadTillId().bare).arg(Logs::b(_history->loadedAtBottom())).arg(showAtMsgId.bare));
	...
	if (peerId)
	{
		...
		_contactStatus->bar().heightValue() | rpl::start_with_next([=]
																   { updateControlsGeometry(); }, _contactStatus->bar().lifetime());
		...
		if (const auto user = _peer->asUser())
		{
			_paysStatus = std::make_unique<PaysStatus>(
				...
				_topBars.get(),
				user);
			_paysStatus->bar().heightValue() | rpl::start_with_next([=]
																	{ updateControlsGeometry(); }, _paysStatus->bar().lifetime());
			_businessBotStatus = std::make_unique<BusinessBotStatus>(
				controller(),
				_topBars.get(),
				user);
			_businessBotStatus->bar().heightValue() | rpl::start_with_next([=]
																		   { updateControlsGeometry(); }, _businessBotStatus->bar().lifetime());
		}
		orderWidgets();
	...
	if (_peer)
	{
		_unblock->setText(((_peer->isUser() && _peer->asUser()->isBot() && !_peer->asUser()->isSupport())
	...
	if (_peer)
	{
		...
		if (_migrated && !_migrated->isEmpty() && (!_history->loadedAtTop() || !_migrated->loadedAtBottom()))
		{
		...
		if (_showAtMsgId == ShowAtUnreadMsgId && !_history->trackUnreadMessages() && !hasSavedScroll())
		{
		...
		_list->sendIntroSticker() | rpl::start_with_next([=](not_null<DocumentData *> sticker)
														 { sendExistingDocument(
															   sticker,
															   Api::MessageToSend(prepareSendAction({}))); }, _list->lifetime());
		_list->show();
		...
		if (const auto channel = _peer->asChannel())
		{
			...
			{
				using Flags = Data::Flags<ChannelDataFlags>;
				channel->flagsValue() | rpl::skip(1) | rpl::start_with_next([=](Flags::Change change)
																			{
		...
		controller()->adaptive().changes() | rpl::start_with_next([=]
																  {
		...
		if (const auto user = _peer->asUser())
		{
			if (const auto &info = user->botInfo)
			{
				if (startBot || (!_history->isEmpty() && clearMaybeSendStart()))
				{
					if (startBot && wasState.key)
					{
						info->inlineReturnTo = wasState;
			...
			else
			{
				Info::Profile::BirthdayValue(
					...
					rpl::flatten_latest() | rpl::distinct_until_changed() | rpl::start_with_next([=]
																								 {
					refreshSendGiftToggle();
					updateControlsVisibility();
					updateControlsGeometry(); }, _list->lifetime());
			}
		}
	...
	if (_history)
	{
		const auto msgId = (_showAtMsgId == ShowAtTheEndMsgId)
							   ? ShowAtUnreadMsgId
							   : _showAtMsgId;
		controller()->setActiveChatEntry({_history,
...
void HistoryWidget::setupPreview()
{
	...
	_preview->repaintRequests() | rpl::start_with_next([=]
													   { updateField(); }, _preview->lifetime());

	_preview->parsedValue() | rpl::start_with_next([=](WebpageParsed value)
												   {
...
void HistoryWidget::refreshAttachBotsMenu()
{
	...
	_attachBotsMenu->heightValue() | rpl::start_with_next([=]
														  { moveFieldControls(); }, _attachBotsMenu->lifetime());
}
...
{
	controller()->activeChatValue() | rpl::map([=](Dialogs::Key key) -> rpl::producer<>
											   {
...
{
	const auto has = _history && _canSendMessages && (session().scheduledMessages().count(_history) > 0);
	if (!_scheduled && has)
	{
...
void HistoryWidget::refreshSendGiftToggle()
{
	...
	const auto all = Type::Premium | Type::Unlimited | Type::Limited | Type::Unique;
	const auto has = user && _canSendMessages && !user->isServiceUser() && !user->isSelf() && !user->isBot() && ((disallowed & Type::SendHide) || (session().user()->disallowedGiftTypes() & Type::SendHide) || Data::IsBirthdayToday(user->birthday())) && ((disallowed & all) != all);
	if (!_giftToUser && has)
	{
	...
	HistoryView::SuggestMode mode)
{
	...
	_suggestOptions->updates() | rpl::start_with_next([=]
													  {
		updateField();
		saveDraftWithTextNow(); }, _suggestOptions->lifetime());
...
{
	const auto has = _peer && _peer->isMonoforum() && !_peer->amMonoforumAdmin();
...
{
	session().sendAsPeers().updated() | rpl::filter([=](not_null<PeerData *> peer)
													{ return (peer == _peer); }) |
		rpl::start_with_next([=]
							 {
		refreshSendAsToggle();
...
{
	return (_attachDragAreas.document->overlaps(globalRect) || _attachDragAreas.photo->overlaps(globalRect) || (_autocomplete && _autocomplete->overlaps(globalRect)) || (_tabbedPanel && _tabbedPanel->overlaps(globalRect)) || (_inlineResults && _inlineResults->overlaps(globalRect)));
}

bool HistoryWidget::canWriteMessage() const
{
	return _history && _canSendMessages && !isBlocked() && !isJoinChannel() && !isMuteUnmute() && !isBotStart() && !isSearching();
}
...
void HistoryWidget::updateControlsVisibility()
{
	...
	if (isChoosingTheme() || (!editingMessage() && (isSearching() || isBlocked() || isJoinChannel() || isMuteUnmute() || isBotStart() || isReportMessages())))
	{
	...
	else if (editingMessage() || _canSendMessages)
	{
		...
		if (_editMsgId || _replyTo || readyToForward() || _previewDrawPreview || _kbReplyTo || _suggestOptions)
		{
...
{
	if (_history != item->history() || !_historyInited || item->isScheduled())
	{
		return;
	}
	if (item->isSponsored())
...
void HistoryWidget::maybeMarkReactionsRead(not_null<HistoryItem *> item)
{
	...
	const auto reactionCenter = view->reactionButtonParameters({}, {}).center.y();
	const auto visibleTop = _scroll->scrollTop();
	const auto visibleBottom = visibleTop + _scroll->height();
	if (itemTop + reactionCenter < visibleTop || itemTop + view->height() > visibleBottom)
	{
		return;
	}
	session().api().markContentsRead(item);
...
void HistoryWidget::unreadCountUpdated()
{
	...
	{
		const auto hideCounter = _history->isForum() || !_history->trackUnreadMessages();
...
{
	if (error.type() == u"CHANNEL_PRIVATE"_q && _peer->isChannel() && _peer->asChannel()->invitePeekExpires())
	{
		_peer->asChannel()->privateErrorReceived();
	}
	else if (error.type() == u"CHANNEL_PRIVATE"_q || error.type() == u"CHANNEL_PUBLIC_GROUP_NA"_q || error.type() == u"USER_BANNED_IN_CHANNEL"_q)
	{
		auto was = _peer;
		closeCurrent();
	...
	LOG(("RPC Error: %1 %2: %3").arg(QString::number(error.code()), error.type(), error.description()));
...
{
	return _history && _list && _historyInited && !_firstLoadRequest && !_delayedShowAtRequest && !_showAnimation && controller()->widget()->markingAsRead();
}
...
void HistoryWidget::loadMessages()
{
	...
	DEBUG_LOG(("JumpToEnd(%1, %2, %3): Loading up before %4.").arg(_history->peer->name()).arg(_history->inboxReadTillId().bare).arg(Logs::b(_history->loadedAtBottom())).arg(offsetId.bare));
...
void HistoryWidget::loadMessagesDown()
{
	...
	const auto loadMigrated = _migrated && !(_migrated->isEmpty() || _migrated->loadedAtBottom() || (!_history->isEmpty() && !_history->loadedAtTop()));
	const auto from = loadMigrated ? _migrated : _history;
	...
	DEBUG_LOG(("JumpToEnd(%1, %2, %3): Loading down after %4.").arg(_history->peer->name()).arg(_history->inboxReadTillId().bare).arg(Logs::b(_history->loadedAtBottom())).arg(offsetId.bare));
	...
	const Window::SectionShow &params)
{
	...
	DEBUG_LOG(("JumpToEnd(%1, %2, %3): Loading delayed around %4.").arg(_history->peer->name()).arg(_history->inboxReadTillId().bare).arg(Logs::b(_history->loadedAtBottom())).arg(showAtMsgId.bare));
...
{
	if (_firstLoadRequest || _delayedShowAtRequest || _scroll->isHidden() || !_peer || !_historyInited)
	{
		return;
	}
	...
	const auto readyForBotStart = hasNonEmpty || (_history->loadedAtTop() && _history->loadedAtBottom());
...
{
	if (!_history || _firstLoadRequest || _preloadRequest || _preloadDownRequest || (_supportPreloadRequest && !force) || controller()->activeChatEntryCurrent().key.history() != _history)
	{
		return;
	}
...
{
	if (_firstLoadRequest || _scroll->isHidden() || !_peer || !_historyInited)
	{
		return;
	}
	auto scrollTop = _scroll->scrollTop();
	...
	{
		auto below = !replyReturn->mainView() && (replyReturn->history() == _history) && !_history->isEmpty() && (replyReturn->id < _history->blocks.back()->messages.back()->data()->id);
		if (!below)
		{
			below = !replyReturn->mainView() && (replyReturn->history() == _migrated) && !_history->isEmpty();
		}
		if (!below)
		{
			below = !replyReturn->mainView() && _migrated && (replyReturn->history() == _migrated) && !_migrated->isEmpty() && (replyReturn->id < _migrated->blocks.back()->messages.back()->data()->id);
		}
		if (!below && replyReturn->mainView())
		{
			below = (scrollTop >= scrollTopMax) || (_list->itemTop(replyReturn) < scrollTop + scrollHeight / 2);
		}
		if (below)
		{
...
void HistoryWidget::saveEditMessage(Api::SendOptions options)
{
	...
	const auto hasMediaWithCaption = item && item->media() && item->media()->allowsEditCaption();
	if (sending.text.isEmpty() && (webPageDraft.removed || webPageDraft.url.isEmpty() || !webPageDraft.manual) && !hasMediaWithCaption)
	{
...
void HistoryWidget::send(Api::SendOptions options)
{
	...
	// If user has typed text, prefer sending text and cancel any active or paused
	// voice recording to avoid duplicate sends (user flow: pause recording ->
	// type -> press mic -> press send can produce spam). Canceling here is
	// defensive and uses public API of VoiceRecordBar.
	if (HasSendText(_field) && _voiceRecordBar)
	{
		if (_voiceRecordBar->isRecording() || _voiceRecordBar->isListenState() || _voiceRecordBar->isActive() || _voiceRecordBar->isRecordingByAnotherBar())
		{
			// Stop (cancel) recording and hide UI immediately.
	...
	_justMarkingAsRead = !HasSendText(_field) && message.webPage.url.isEmpty();
...
{
	if (!_peer || !_peer->isUser() || !_peer->asUser()->isBot() || !_canSendMessages)
	{
		updateControlsVisibility();
		return;
	}
	...
	Data::MessagePosition position)
{
	...
	{
		DEBUG_LOG(("JumpToEnd(%1, %2, %3): Show at unread requested.").arg(_history->peer->name()).arg(_history->inboxReadTillId().bare).arg(Logs::b(_history->loadedAtBottom())));
...
{
	return _peer && _sendPayment.check(
						controller(),
						_peer,
						options,
						messagesCount,
...
void HistoryWidget::sendButtonClicked()
{
	...
	else if (type != Ui::SendButton::Type::Record && type != Ui::SendButton::Type::Round)
	{
		send({});
	}
}
...
void HistoryWidget::updateOverStates(QPoint pos)
{
	...
	const auto inDetails = detailsRect.contains(pos) && (_editMsgId || replyTo() || isReadyToForward || hasWebPage || _suggestOptions);
	const auto inPhotoEdit = inDetails && _photoEditMedia && QRect(detailsRect.x() + st::historyReplySkip, (detailsRect.y() + (detailsRect.height() - st::historyReplyPreview) / 2), st::historyReplyPreview, st::historyReplyPreview).contains(pos);
	...
	Api::SendOptions options)
{
	...
	const auto lastKeyboardUsed = (forMsgId == request.replyTo.messageId) && (forMsgId == FullMsgId(_peer->id, _history->lastKeyboardId));
...
void HistoryWidget::hideSingleUseKeyboard(FullMsgId replyToId)
{
	...
	const auto lastKeyboardUsed = (_keyboard->forMsgId() == replyToId) && (_keyboard->forMsgId() == FullMsgId(_peer->id, _history->lastKeyboardId));
	if (replyToId)
	{
...
bool HistoryWidget::insertBotCommand(const QString &cmd)
{
	...
	if (!insertingInlineBot)
	{
		...
		const auto m = QRegularExpression(
						   u"^/[A-Za-z_0-9]{0,64}(@[A-Za-z_0-9]{0,32})?(\\s|$)"_q)
						   .match(
							   textWithTags.text);
...
{
	return _peer && _peer->isBroadcast() && Data::CanSendAnything(_peer) && !session().data().notifySettings().silentPostsUnknown(_peer);
}
...
{
	return _peer && ((_peer->isBroadcast() && !_peer->asChannel()->canPostMessages()) || (_peer->isGigagroup() && !Data::CanSendAnything(_peer)) || _peer->isRepliesChat() || _peer->isVerifyCodes());
}
...
{
	return (_recordAvailability != Webrtc::RecordAvailability::None) && !_voiceRecordBar->isListenState() && !_voiceRecordBar->isRecordingByAnotherBar() && !HasSendText(_field) && !_previewDrawPreview && (_replyTo || !readyToForward()) && !_editMsgId;
}
...
void HistoryWidget::updateSendButtonType()
{
	...
	const auto disabledBySlowmode = _peer && _peer->slowmodeApplied() && (_history->latestSendingMessage() != nullptr);
	const auto delay = [&]
	{
	...
	_send->setDisabled(disabledBySlowmode && (type == Type::Send || type == Type::Record || type == Type::Round));
...
bool HistoryWidget::updateCmdStartShown()
{
	...
	if (_history && _peer && (false || (_peer->isChat() && _peer->asChat()->botStatus > 0) || (_peer->isMegagroup() && _peer->asChannel()->mgInfo->botStatus > 0)))
	{
	...
	if (!bot || (bot->botInfo->botMenuButtonUrl.isEmpty() && bot->botInfo->commands.empty()))
	{
	...
	else if (!_botMenu.button)
	{
		...
		_botMenu.button->widthValue() | rpl::start_with_next([=](int width)
															 {
	...
	const auto textChanged = _botMenu.button && ((_botMenu.text != bot->botInfo->botMenuButtonText) || (_botMenu.small != textSmall));
	if (textChanged)
	{
		_botMenu.text = bot->botInfo->botMenuButtonText;
...
{
	return _history && (_keyboard->forMsgId() == FullMsgId(
													 _history->peer->id,
...
{
	_botKeyboardHide->setVisible(!_peer->isUser() || !_keyboard->persistent());
}
...
void HistoryWidget::toggleKeyboard(bool manual)
{
	...
	if (_kbShown || _kbReplyTo)
	{
		...
		if (_kbShown)
		{
			...
			_kbReplyTo = nullptr;
			if (!readyToForward() && !_previewDrawPreview && !_editMsgId && !_replyTo && !_suggestOptions)
			{
				_fieldBarCancel->hide();
				updateMouseTracking();
			}
	...
	else if (!_keyboard->hasMarkup() && _keyboard->forceReply())
	{
		...
		_kbReplyTo = (false || _peer->isChat() || _peer->isChannel() || _keyboard->forceReply())
...
void HistoryWidget::toggleTabbedSelectorMode()
{
	...
	if (_tabbedPanel)
	{
		if (controller()->canShowThirdSection() && !controller()->adaptive().isOneColumn())
		{
...
{
	const auto kbShowShown = _history && !_kbShown && _keyboard->hasMarkup();
	auto fieldWidth = width() - _attachToggle->width() - st::historySendRight - _send->width() - _tabbedSelectorToggle->width();
	if (_botMenu.button)
	{
...
{
	_voiceRecordBar->setPauseInsteadSend(_history && _history->peer->starsPerMessageChecked() > 0);
	...
	std::optional<bool> compress) const
{
	const auto error = [&]() -> Data::SendError
	{
		...
		case Error::NonLocalUrl:
			return tr::lng_send_image_empty(
				tr::now,
				lt_name,
				list.errorData);
		case Error::TooLargeFile:
			return u"(toolarge)"_q;
		}
...
MsgId HistoryWidget::resolveReplyToTopicRootId()
{
	...
	if (result && _peer->isForum() && !_peer->forumTopicFor(result) && _topicsRequested.emplace(result).second)
	{
	...
	Api::SendOptions options)
{
	...
	return withPaymentApproved && !checkSendPayment(
	...
	const QString &insertTextOnCancel)
{
	...
	if (const auto urls = Core::ReadMimeUrls(data); !urls.empty())
	{
		...
		{
			if (list.error == Ui::PreparedList::Error::None || !hasImage)
			{
...
void HistoryWidget::handleHistoryChange(not_null<const History *> history)
{
	if (_list && (_history == history || _migrated == history))
	{
		...
		if (!_scroll->isHidden())
		{
			...
			const auto update = false || (_reportMessages->isHidden() == reportMessages) || (!reportMessages && _unblock->isHidden() == unblock) || (!reportMessages && !unblock && _botStart->isHidden() == botStart) || (!reportMessages && !unblock && !botStart && _joinChannel->isHidden() == joinChannel) || (!reportMessages && !unblock && !botStart && !joinChannel && _muteUnmute->isHidden() == muteUnmute);
			if (update)
			{
				updateControlsVisibility();
				updateControlsGeometry();
...
void HistoryWidget::updateControlsGeometry()
{
	...
	_topBars->move(tabsLeftSkip, _topBar->bottomNoMargins() + (_subsectionTabs ? _subsectionTabs->topSkip() : 0));
	...
	const auto sponsoredMessageBarTop = pinnedBarTop + (_pinnedBar ? _pinnedBar->height() : 0);
	if (_sponsoredMessageBar)
	{
		_sponsoredMessageBar->move(0, sponsoredMessageBarTop);
		_sponsoredMessageBar->resizeToWidth(innerWidth);
	}
	const auto translateTop = sponsoredMessageBarTop + (_sponsoredMessageBar ? _sponsoredMessageBar->height() : 0);
	if (_translateBar)
	{
		_translateBar->move(0, translateTop);
		_translateBar->resizeToWidth(innerWidth);
	}
	const auto paysStatusTop = translateTop + (_translateBar ? _translateBar->height() : 0);
	if (_paysStatus)
	{
		_paysStatus->bar().move(0, paysStatusTop);
	}
	const auto contactStatusTop = paysStatusTop + (_paysStatus ? _paysStatus->bar().height() : 0);
	if (_contactStatus)
	{
	...
	const auto scrollAreaTop = _topBars->y() + businessBotTop + (_businessBotStatus ? _businessBotStatus->bar().height() : 0);
	_topBars->resize(
		innerWidth,
...
{
	const auto checked = skipNoAdminCheck || (_history && _history->suggestDraftAllowed());
...
bool HistoryWidget::hasSavedScroll() const
{
	...
	return _history->scrollTopItem || (_migrated && _migrated->scrollTopItem);
}
...
int HistoryWidget::countInitialScrollTop()
{
	...
	else if (_showAtMsgId && (IsServerMsgId(_showAtMsgId) || IsClientMsgId(_showAtMsgId) || IsServerMsgId(-_showAtMsgId)))
	{
...
int HistoryWidget::countAutomaticScrollTop()
{
	...
	if (const auto unread = _history->firstUnreadMessage())
	{
		...
		const auto possibleUnreadBarTop = _scroll->scrollTopMax() + HistoryView::UnreadBar::height() - HistoryView::UnreadBar::marginTop();
...
{
	if (!_canSendMessages && _peer->amMonoforumAdmin() && !_peer->asChannel()->monoforumDisabled())
	{
	...
	const auto allWithoutPolls = Data::AllSendRestrictions() & ~ChatRestriction::SendPolls;
	...
	const ScrollChange &change)
{
	...
	const auto newScrollWidth = width() - (_subsectionTabs ? _subsectionTabs->leftSkip() : 0);
	const auto subsectionTabsTop = _topBar->bottomNoMargins();
	auto newScrollHeight = height() - subsectionTabsTop - (_subsectionTabs ? _subsectionTabs->topSkip() : 0);
	if (_translateBar)
	{
	...
	else if (!editingMessage() && (isSearching() || isBlocked() || isBotStart() || isJoinChannel() || isMuteUnmute() || isReportMessages()))
	{
	...
	else
	{
		...
		if (_editMsgId || replyTo() || readyToForward() || _previewDrawPreview || _suggestOptions)
		{
	...
	const auto needResize = (_scroll->width() != newScrollWidth) || (_scroll->height() != newScrollHeight);
	if (needResize)
	{
...
void HistoryWidget::revealItemsCallback()
{
	...
	for (auto i = begin(_itemRevealAnimations); i != end(_itemRevealAnimations);)
	{
...
std::optional<int> HistoryWidget::unreadBarTop() const
{
	...
	if (bar)
	{
		const auto result = _list->itemTop(bar) + HistoryView::UnreadBar::marginTop();
...
void HistoryWidget::updateBotKeyboard(History *h, bool force)
{
	...
	const auto forceReply = _keyboard->forceReply() && (!_replyTo || !_replyEditMsg);
	if (hasMarkup || forceReply)
	{
		if (_keyboard->singleUse() && _keyboard->hasMarkup() && (_keyboard->forMsgId() == FullMsgId(_history->peer->id, _history->lastKeyboardId)) && _history->lastKeyboardUsed)
		{
		...
		else
		{
			...
			_kbShown = false;
			_kbReplyTo = nullptr;
			if (!readyToForward() && !_previewDrawPreview && !_replyTo && !_suggestOptions)
			{
				_fieldBarCancel->hide();
				updateMouseTracking();
			}
	...
	else
	{
		...
		_kbShown = false;
		_kbReplyTo = nullptr;
		if (!readyToForward() && !_previewDrawPreview && !_replyTo && !_editMsgId && !_suggestOptions)
		{
			_fieldBarCancel->hide();
			updateMouseTracking();
		}
...
void HistoryWidget::botCallbackSent(not_null<HistoryItem *> item)
{
	...
	const auto lastKeyboardUsed = (keyId == FullMsgId(_peer->id, item->id)) && (keyId == FullMsgId(_peer->id, _history->lastKeyboardId));
...
{
	const auto available = height() - _topBar->height() - (_paysStatus ? _paysStatus->bar().height() : 0) - (_contactStatus ? _contactStatus->bar().height() : 0) - (_businessBotStatus ? _businessBotStatus->bar().height() : 0) - (_sponsoredMessageBar ? _sponsoredMessageBar->height() : 0) - (_pinnedBar ? _pinnedBar->height() : 0) - (_groupCallBar ? _groupCallBar->height() : 0) - (_requestsBar ? _requestsBar->height() : 0) - ((_editMsgId || replyTo() || readyToForward() || _previewDrawPreview) ? st::historyReplyHeight : 0) - (2 * st::historySendPadding) - st::historyReplyHeight; // at least this height for history.
...
std::optional<bool> HistoryWidget::cornerButtonsDownShown()
{
	...
	if (_voiceRecordBar->isLockPresent() || _voiceRecordBar->isTTLButtonShown())
	{
		return false;
	}
	...
	{
		if (!_list || !history || history->unreadCount() <= 0 || !history->trackUnreadMessages())
		{
			return false;
		}
...
void HistoryWidget::mousePressEvent(QMouseEvent *e)
{
	...
	const auto isReadyToForward = readyToForward();
	if (_editMsgId && (_inDetails || _inPhotoEdit) && (e->button() == Qt::RightButton))
	{
	...
	else if (_replyTo && ((e->modifiers() & Qt::ControlModifier) || (e->button() != Qt::LeftButton)))
	{
		jumpToReply(_replyTo);
	}
	else if (_replyTo || (isReadyToForward && e->button() == Qt::LeftButton))
	{
		editDraftOptions();
	}
...
void HistoryWidget::keyPressEvent(QKeyEvent *e)
{
	...
	else if (e->key() == Qt::Key_Up && !commonModifiers)
	{
		const auto item = _history
							  ...
							  : nullptr;
		if (item && _field->empty() && !_editMsgId && !_replyTo)
		{
			editMessage(item, {});
			return;
		}
	...
	else if (e->key() == Qt::Key_Up && commonModifiers == Qt::ControlModifier)
	{
	...
	else if (e->key() == Qt::Key_Down && commonModifiers == Qt::ControlModifier)
	{
...
void HistoryWidget::handlePeerMigration()
{
	...
	if (from && to && !from->isEmpty() && (!from->loadedAtBottom() || !to->loadedAtTop()))
	{
...
{
	if (!_history || _editMsgId || _history->isForum() || (_replyTo && _replyTo.messageId.peer != _history->peer->id))
	{
		return false;
	}
...
{
	if (_firstLoadRequest || _delayedShowAtRequest || _scroll->isHidden() || !_history || !_historyInited || !_pinnedTracker)
	{
		return;
	}
	const auto visibleBottom = _scroll->scrollTop() + _scroll->height();
	...
	if (_pinnedClickedId && lessThanId <= lastClickedId && !_scrollToAnimation.animating())
	{
		_pinnedClickedId = FullMsgId();
	}
...
void HistoryWidget::checkLastPinnedClickedIdReset(
	...
	int nowScrollTop)
{
	if (_firstLoadRequest || _delayedShowAtRequest || _scroll->isHidden() || !_history || !_historyInited)
	{
		return;
	}
...
void HistoryWidget::setupTranslateBar()
{
	...
	controller()->adaptive().oneColumnValue() | rpl::start_with_next([=, raw = _translateBar.get()](bool one)
																	 { raw->setShadowGeometryPostprocess([=](QRect geometry)
																										 {
			if (!one) {
				geometry.setLeft(geometry.left() + st::lineWidth);
			}
			return geometry; }); }, _translateBar->lifetime());

	_translateBarHeight = 0;
	_translateBar->heightValue() | rpl::start_with_next([=](int height)
														{
		_topDelta = _preserveScrollTop ? 0 : (height - _translateBarHeight);
		_translateBarHeight = height;
		updateHistoryGeometry();
		updateControlsGeometry();
		_topDelta = 0; }, _translateBar->lifetime());
...
void HistoryWidget::checkPinnedBarState()
{
	...
	auto pinnedRefreshed = Info::Profile::SharedMediaCountValue(
						   ...
						   rpl::distinct_until_changed() | rpl::map([=](int count)
																	{
		if (_pinnedClickedId) {
			_pinnedClickedId = FullMsgId();
			_minPinnedId = std::nullopt;
	...
	rpl::combine(
		...
		rpl::start_with_next([=](bool many, HistoryItem *item)
							 { refreshPinnedBarButton(many, item); }, _pinnedBar->lifetime());
	...
	controller()->adaptive().oneColumnValue() | rpl::start_with_next([=, raw = _pinnedBar.get()](bool one)
																	 { raw->setShadowGeometryPostprocess([=](QRect geometry)
																										 {
			if (!one) {
				geometry.setLeft(geometry.left() + st::lineWidth);
			}
			return geometry; }); }, _pinnedBar->lifetime());

	_pinnedBar->barClicks() | rpl::start_with_next([=]
												   {
	...
	_pinnedBarHeight = 0;
	_pinnedBar->heightValue() | rpl::start_with_next([=](int height)
													 {
		_topDelta = _preserveScrollTop ? 0 : (height - _pinnedBarHeight);
		_pinnedBarHeight = height;
		updateHistoryGeometry();
		updateControlsGeometry();
		_topDelta = 0; }, _pinnedBar->lifetime());
...
void HistoryWidget::refreshPinnedBarButton(bool many, HistoryItem *item)
{
	...
	if (customButton)
	{
		...
		_pinnedBar->contextMenuRequested() | rpl::start_with_next([=]
																  {
			state->menu = base::make_unique_q<Ui::PopupMenu>(buttonRaw);
			state->menu->addAction(
				tr::lng_settings_events_pinned(tr::now),
				openSection);
			state->menu->popup(QCursor::pos()); }, buttonRaw->lifetime());
...
void HistoryWidget::setupGroupCallBar()
{
	...
	controller()->adaptive().oneColumnValue() | rpl::start_with_next([=](bool one)
																	 { _groupCallBar->setShadowGeometryPostprocess([=](QRect geometry)
																												   {
			if (!one) {
				geometry.setLeft(geometry.left() + st::lineWidth);
			}
			return geometry; }); }, _groupCallBar->lifetime());
	...
	_groupCallBar->heightValue() | rpl::start_with_next([=](int height)
														{
		_topDelta = _preserveScrollTop ? 0 : (height - _groupCallBarHeight);
		_groupCallBarHeight = height;
		updateHistoryGeometry();
		updateControlsGeometry();
		_topDelta = 0; }, _groupCallBar->lifetime());
...
void HistoryWidget::setupRequestsBar()
{
	...
	controller()->adaptive().oneColumnValue() | rpl::start_with_next([=](bool one)
			...
			return geometry; }); }, _requestsBar->lifetime());

	_requestsBar->barClicks() | rpl::start_with_next([=]
													 { RequestsBoxController::Start(controller(), _peer); }, _requestsBar->lifetime());

	_requestsBarHeight = 0;
	_requestsBar->heightValue() | rpl::start_with_next([=](int height)
													   {
		_topDelta = _preserveScrollTop ? 0 : (height - _requestsBarHeight);
		_requestsBarHeight = height;
		updateHistoryGeometry();
		updateControlsGeometry();
		_topDelta = 0; }, _requestsBar->lifetime());
...
{
	const auto h = _list->height() - (_kbScroll->isHidden() ? 0 : _kbScroll->height());
	...
	const auto checkState = [=, this]
	{
		...
		if (state == State::AppendToTopBar)
		{
			...
			else
			{
				...
				_list->heightValue() | rpl::start_with_next([=, this]
															{
...
void HistoryWidget::createSponsoredMessageBar()
{
	...
	session().sponsoredMessages().itemRemoved(
		...
		_sponsoredMessageBar->shownValue() | rpl::filter(
			...
			_sponsoredMessageBar = nullptr;
		}, _sponsoredMessageBar->lifetime()); }, _sponsoredMessageBar->lifetime());

	if (maybeFullId)
	{
		const auto viewLifetime = _sponsoredMessageBar->lifetime().make_state<rpl::lifetime>();
		rpl::combine(
			...
			viewLifetime->destroy(); }, *viewLifetime);
	}

	_sponsoredMessageBarHeight = 0;
	_sponsoredMessageBar->heightValue() | rpl::start_with_next([=](int height)
															   {
		...
		updateHistoryGeometry();
		updateControlsGeometry();
		_topDelta = 0; }, _sponsoredMessageBar->lifetime());
	_sponsoredMessageBar->toggle(false, anim::type::instant);
}
	...
	std::optional<MsgId> localId)
{
	...
	else if (!_peer || !_canSendMessages || showSlowmodeError() || ShowSendPremiumError(controller(), document))
	{
		return false;
	}
	const auto withPaymentApproved = [=](int approved)
...
{
	if (!_subsectionCheckLifetime && _history && _history->peer->isMegagroup())
	{
		_subsectionCheckLifetime = _history->peer->asChannel()->flagsValue() | rpl::skip(1) | rpl::filter([=](Data::Flags<ChannelDataFlags>::Change change)
																										  {
...
void HistoryWidget::checkCharsLimitation()
{
	...
	const auto hasMediaWithCaption = item->media() && item->media()->allowsEditCaption();
	const auto maxCaptionSize = !hasMediaWithCaption
									? MaxMessageSize
									: Data::PremiumLimits(&session()).captionLengthCurrent();
	const auto remove = _fieldCharsCountManager.count() - maxCaptionSize;
	if (remove > 0)
	{
		if (!_charsLimitation)
		{
			...
			Data::AmPremiumValue(
				&session()) |
				rpl::start_with_next([=]
									 { checkCharsLimitation(); }, _charsLimitation->lifetime());
		}
...
void HistoryWidget::processReply()
{
	...
	else if (const auto forum = _peer->forum(); forum && _processingReplyItem->history() == _history)
	{
...
{
	return _peer && (replyTo.peer == _peer->id) && _keyboard->forceReply() && (_keyboard->forMsgId() == FullMsgId(_peer->id, _history->lastKeyboardId)) && _keyboard->forMsgId().msg == replyTo.msg;
}

bool HistoryWidget::lastForceReplyReplied() const
{
	return _peer && _keyboard->forceReply() && _keyboard->forMsgId() == replyTo().messageId && (_keyboard->forMsgId() == FullMsgId(_peer->id, _history->lastKeyboardId));
}
...
bool HistoryWidget::cancelReply(bool lastKeyboardUsed)
{
	...
	if (_replyTo)
	{
		...
		_processingReplyTo = _replyTo = FullReplyTo();
		mouseMoveEvent(0);
		if (!readyToForward() && !_previewDrawPreview && !_kbReplyTo && !_suggestOptions)
		{
			_fieldBarCancel->hide();
			updateMouseTracking();
		}
...
void HistoryWidget::cancelEdit()
{
	...
	mouseMoveEvent(nullptr);
	if (!readyToForward() && !_previewDrawPreview && !replyTo() && !_suggestOptions)
	{
		_fieldBarCancel->hide();
		updateMouseTracking();
	}
...
void HistoryWidget::handlePeerUpdate()
{
	...
	if (!_showAnimation)
	{
		...
		if (blockChanged || (!isBlocked() && (_joinChannel->isHidden() == isJoinChannel())))
		{
			resize = true;
		}
		if (updateCanSendMessage())
...
bool HistoryWidget::updateCanSendMessage()
{
	...
	const auto restrictedOnlyReplies = onlyReplies && (!_replyTo.messageId || _replyTo.messageId.peer != _peer->id);
	...
	if (_canSendMessages == newCanSendMessages && _canSendTexts == newCanSendTexts)
	{
		return false;
	}
...
void HistoryWidget::escape()
{
	...
	{
		if (_replyEditMsg && EditTextChanged(_replyEditMsg, _field->getTextWithTags()))
		{
	...
	else if ((_replyTo || _suggestOptions) && _field->getTextWithTags().empty())
	{
		cancelReplyOrSuggest();
	}
...
void HistoryWidget::updateTopBarSelection()
{
	...
	updateControlsVisibility();
	updateHistoryGeometry();
	if (!controller()->isLayerShown() && !Core::App().passcodeLocked())
	{
		if (isSearching() && !_nonEmptySelection)
		{
			_composeSearch->setInnerFocus();
		}
		else if (_nonEmptySelection || (_list && _list->wasSelectedText()) || isRecording() || isBotStart() || isBlocked() || (!_canSendTexts && !_editMsgId))
		{
			_list->setFocus();
		}
		else
	...
	MsgId msgId)
{
	...
	else if (_editMsgId == msgId || (_replyTo.messageId == FullMsgId(peer->id, msgId)))
	{
		updateReplyEditTexts(true);
		if (_editMsgId == msgId)
		{
...
void HistoryWidget::updateReplyEditTexts(bool force)
{
	...
	if (_replyEditMsg)
	{
		...
		_photoEditMedia = (_canReplaceMedia && editMedia->photo() && !editMedia->photo()->isNull())
...
void HistoryWidget::drawField(Painter &p, const QRect &rect)
{
	...
	if (_editMsgId || _replyTo || hasForward || _kbReplyTo || _previewDrawPreview || _suggestOptions)
	{
	...
	if (_previewDrawPreview)
	{
		...
		const auto elidedWidth = width() - previewLeft - _fieldBarCancel->width() - st::msgReplyPadding.right();
	...
	else if (_editMsgId || _replyTo || (!hasForward && _kbReplyTo))
	{
		...
		if (drawMsgText)
		{
			if (hasPreview)
			{
				if (preview)
				{
					...
					auto to = QRect(
						replyLeft,
						(st::historyReplyHeight - st::historyReplyPreview) / 2 + backy,
						st::historyReplyPreview,
						st::historyReplyPreview);
					p.drawPixmap(to.x(), to.y(), preview->pixSingle(preview->size() / style::DevicePixelRatio(), {
			...
			else
			{
				...
				_replyEditMsgText.draw(p, {
											  .position = QPoint(
												  replyLeft,
												  st::msgReplyPadding.top() + st::msgServiceNameFont->height + backy),
		...
		else
		{
			...
			p.drawText(
				replyLeft,
				backy + (st::historyReplyHeight - st::msgDateFont->height) / 2 + st::msgDateFont->ascent,
	...
	else if (hasForward)
	{
		...
		const auto available = width() - x - _fieldBarCancel->width() - st::msgReplyPadding.right();
	...
	int top) const
{
	...
	if (!_replyEditMsg || _replyEditMsg->history()->peer->canEditMessagesIndefinitely())
	{
		return;
	}
	...
	if (!editTimeLeftText.isEmpty())
	{
		...
		p.setPen(st::historyComposeAreaFgService);
		p.drawText(
			left + st::msgServiceNameFont->width(tr::lng_edit_message(tr::now)) + st::normalFont->spacew,
...
{
	if (paintShowAnimationFrame() || controller()->contentOverlapped(this, e))
	{
		return;
	}
	if (hasPendingResizedItems())
	...
	if (_list)
	{
		const auto restrictionHidden = fieldOrDisabledShown() || isRecording();
		if (restrictionHidden || replyTo() || readyToForward() || _kbShown || _suggestOptions)
		{
			if (!isSearching())
			{
				drawField(p, clip);
	...
	{
		const auto w = 0 + st::msgServiceFont->width(tr::lng_willbe_history(tr::now)) + st::msgPadding.left() + st::msgPadding.right();
		const auto h = st::msgServiceFont->height + st::msgServicePadding.top() + st::msgServicePadding.bottom();
		const auto tr = QRect(
			(width() - w) / 2,
			st::msgServiceMargin.top() + (height() - fieldHeight() - 2 * st::historySendPadding - h - st::msgServiceMargin.top() - st::msgServiceMargin.bottom()) / 2,
			w,
			h);
