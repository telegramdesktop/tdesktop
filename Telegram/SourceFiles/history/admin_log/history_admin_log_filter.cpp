...
namespace AdminLog
{
	namespace
	{
		...
		UserCheckbox::UserCheckbox(QWidget* parent, not_null<UserData*> user, bool checked) :
			...
			, _check(std::make_unique<Ui::CheckView>(st::defaultCheck, checked, [this]
			{
	...
	FilterBox::FilterBox(QWidget*, not_null<ChannelData*> channel, const std::vector<not_null<UserData*>>& admins, const FilterValue& filter, Fn<void(FilterValue&& filter)> saveCallback) :
		BoxContent()
		, _channel(channel)
		, _admins(admins)
	...
	void FilterBox::prepare()
	{
		...
		_inner = setInnerWidget(object_ptr<Inner>(this, _channel, _admins, _initialFilter, [this]
		{
			refreshButtons();
		}));
