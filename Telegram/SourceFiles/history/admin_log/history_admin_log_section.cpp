...
namespace AdminLog
{
		...
		, _filter(this, langFactory(lng_admin_log_filter), st::topBarButton)
	{
		...
		_filter->setClickedCallback([=]
		{
			showFilterSignal.notify();
		});
		_search->setClickedCallback([=]
		{
			showSearch();
		});
		...
		_field->hide();
		connect(_field, &Ui::FlatInput::cancelled, [=]
		{
			cancelSearch();
		});
		connect(_field, &Ui::FlatInput::changed, [=]
		{
			searchUpdated();
		});
		connect(_field, &Ui::FlatInput::submitted, [=]
		{
			applySearch();
		});
		_searchTimer.setCallback([=]
		{
			applySearch();
		});
	...
	Widget::Widget(QWidget* parent, not_null<Window::SessionController*> controller, not_null<ChannelData*> channel) :
		...
		, _whatIsThis(this, lang(lng_admin_log_about).toUpper(), st::historyComposeButton)
	{
		...
		subscribe(_fixedBar->showFilterSignal, [this]
		{
			showFilter();
		});
		subscribe(_fixedBar->searchCancelledSignal, [this]
		{
			setInnerFocus();
		});
		subscribe(_fixedBar->searchSignal, [this](const QString& query)
		{
		...
		subscribe(_inner->showSearchSignal, [this]
		{
			_fixedBar->showSearch();
		});
		subscribe(_inner->cancelledSignal, [this]
		{
			_fixedBar->goBack();
		});
		subscribe(_inner->scrollToSignal, [this](int top)
		{
		...
		connect(_scroll, &Ui::ScrollArea::scrolled, this, [this]
		{
			onScroll();
		});
