		...
		if (_call)
		{
				...
				&& (state != State::Failed))
			{
				...
				if (_hangupShown != hangupShown)
				{
					...
					_hangupShownProgress.start([this]
					{
						updateHangupGeometry();
					}, _hangupShown ? 0. : 1., _hangupShown ? 1. : 0., st::callPanelDuration, anim::sineInOut);
				}
	...
	void Panel::updateStatusText(State state)
	{
		auto statusText = [this, state]() -> QString
		{
			...
			case State::WaitingInitAck:
				return lang(lng_call_status_connecting);
			...
			case State::Failed:
				return lang(lng_call_status_failed);
			...
			case State::WaitingIncoming:
				return lang(lng_call_status_incoming);
			case State::Ringing:
				return lang(lng_call_status_ringing);
			case State::Busy:
				return lang(lng_call_status_busy);
			}
