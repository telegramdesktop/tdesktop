...
namespace Calls
{
	...
	Call::Call(
		...
		, _type(type)
	{
		_discardByTimeoutTimer.setCallback([this]
		{
			hangup();
		});
	...
	void Call::hangup()
	{
		...
		else
		{
			...
			auto reason = missed ? MTP_phoneCallDiscardReasonMissed() : declined ? MTP_phoneCallDiscardReasonBusy() : MTP_phoneCallDiscardReasonHangup();
	...
	void Call::setState(State state)
	{
		...
		if (_state != state)
		{
			...
			case State::ExchangingKeys:
				...
				break;
			case State::Ended:
				_delegate->playSound(Delegate::Sound::Ended);[[fallthrough]];
