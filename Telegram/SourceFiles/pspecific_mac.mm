...
void PsMainWindow::psNotify(History *history, MsgId msgId) {
	...
	if (notifyHistories.constFind(history) != notifyHistories.cend()) return;
	notifyHistories.insert(history);
    psShowNextNotify();*/
}
...
void PsMainWindow::psClearNotify(History *history) {
    /*if (!history) {
		...
		for (NotifyHistories::const_iterator i = notifyHistories.cbegin(), e = notifyHistories.cend(); i != e; ++i) {
			(*i)->clearNotifyFrom();
		}
		notifyHistories.clear();
		return;
	}
	notifyHistories.remove(history);
	for (PsNotifyWindows::const_iterator i = notifyWindows.cbegin(), e = notifyWindows.cend(); i != e; ++i) {
...
/*void PsMainWindow::psShowNextNotify(PsNotifyWindow *remove) {
	...
	QRect r = QApplication::desktop()->availableGeometry(App::wnd());
	int32 x = r.width() - st::notifyWidth - st::notifyDeltaX, y = r.bottom() - st::notifyHeight - st::notifyDeltaY;
	for (PsNotifyWindows::const_iterator i = notifyWindows.cbegin(), e = notifyWindows.cend(); i != e; ++i) {
		int32 ind = (*i)->index();
		if (ind < 0) continue;
		--count;
	...
	while (count > 0) {
		HistoryItem *notify = 0;
		for (NotifyHistories::iterator i = notifyHistories.begin(), e = notifyHistories.end(); i != e;) {
			if ((*i)->notifyFrom) {
				if (!notify || (*i)->notifyFrom->date < notify->date) {
					notify = (*i)->notifyFrom;
				}
				++i;
			} else {
				i = notifyHistories.erase(i);
			}
		}
		if (notify) {
			notifyWindows.push_back(new PsNotifyWindow(notify, x, y));
			notify->history()->getNextNotifyFrom();
			--count;
		} else {
			break;
		}
