...
void PaintRowTopRight(Painter &p, const QString &text, QRect &rectForName, bool active, bool selected, int outerw = -1) {
	...
	if (rtl() && outerw > 0) left = outerw - left - width;
...
void PaintNarrowCounter(
	...
	if (displayUnreadCounter || displayUnreadMark) {
		...
		const auto badge = PaintUnreadBadge(
			...
			unreadTop,
			st,
			allowDigits); /**paints unread counter or mark badge in narrow dialogs*/
...
int PaintWideCounter(
	...
	if (displayUnreadCounter || displayUnreadMark) {
		...
		const auto unreadRight = fullWidth
			- st::dialogsPadding.x(); /**unread counter or unread mark badge coordination*/
		const auto unreadLeft = st::dialogsPadding.x();
		...
		const auto badge = PaintUnreadBadge(
			...
			rtl() ? unreadLeft : unreadRight,
			unreadTop,
			st); /**paints unread counter or unread mark badge*/
	...
	if (displayMentionBadge || displayReactionBadge) {
		const auto counter = QString();
		const auto unreadRight = fullWidth /**mention or reaction badge coordination*/
			- st::dialogsPadding.x()
...
void paintRow(
	...
	if (flags & Flag::SavedMessages) {
		...
		hiddenSenderInfo->userpic.paint(
			...
			fullWidth,
			st::dialogsPhotoSize);
	} else { /**Archived Chats folder goes here*/
	...
	if (promoted && !history->topPromotionMessage().isEmpty()) {
		...
		if (history && !promoted) {
			PaintRowDate(p, date, rectForName, active, selected, fullWidth); /**paints date*/
		}
	...
	if (flags & (Flag::SavedMessages | Flag::RepliesMessages)) {
		...
		p.setPen(active
			...
			? st::dialogsNameFgOver
			: st::dialogsNameFg);
		from->nameText().drawLeftElided(p, rectForName.left(), rectForName.top(), rectForName.width(), fullWidth, 1); //draw chat's name
	} else if (hiddenSenderInfo) {
		p.setPen(active
			? st::dialogsNameFgActive
			: selected
			? st::dialogsNameFgOver
			: st::dialogsNameFg);
		hiddenSenderInfo->nameText.drawLeftElided(p, rectForName.left(), rectForName.top(), rectForName.width(), fullWidth); //draw chat's name when hidden sender info exists
	} else {
		p.setPen(active
			? st::dialogsNameFgActive
			: selected
...
void PaintCollapsedRow(
	...
	if (!narrow || !folder) {
		...
		p.drawText(rtl() ? right : left, textBaseline, text); /**draws achived chats title when collapsed*/
	} else {
