...
namespace InlineBots
{
	...
	Result::Result(const Creator& creator) :
		_queryId(creator.queryId), _type(creator.type)
	{
	}
	...
	std::unique_ptr<Result> Result::create(uint64 queryId, const MTPBotInlineResult& mtpData)
	{
		...
		static StaticNeverFreedPointer<StringToTypeMap> stringToTypeMap{
			([]() -> StringToTypeMap*
			{
		...
		auto getInlineResultType = [](const MTPBotInlineResult& inlineResult) -> Type
		{
			...
			case mtpc_botInlineResult:
				type = qs(inlineResult.c_botInlineResult().vtype);
				break;
			case mtpc_botInlineMediaResult:
				type = qs(inlineResult.c_botInlineMediaResult().vtype);
				break;
			}
		...
		const MTPstring& mimeType)
	{
		...
		const auto find = [&](mtpTypeId attributeType)
		{
			return ranges::find(
				result,
				attributeType,
				[](const MTPDocumentAttribute& value)
				{
					return value.type();
				});
		};
