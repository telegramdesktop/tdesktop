...
namespace InlineBots
{
	namespace Layout
	{
		namespace internal
		{
			...
			void Inner::refreshSwitchPmButton(const CacheEntry* entry)
			{
				...
				else
				{
					...
					_switchPmButton->setText([text]
					{
						return text;
					}); // doesn't perform text.toUpper()
			...
			int Inner::validateExistingInlineRows(const Results& results)
			{
				...
				{
					// remove last row, maybe it is not full
					--untilrow;
		...
		void Widget::startOpacityAnimation(bool hiding)
		{
			...
			}, _hiding ? 1. : 0., _hiding ? 0. : 1., st::emojiPanDuration);
		}
