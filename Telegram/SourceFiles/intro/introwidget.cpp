...
namespace Intro
{
		...
		, _next(this, Fn<QString()>(), st::introNextButton)
	{
		...
		_back->entity()->setClickedCallback([this]
		{
		...
		_settings->entity()->setClickedCallback([]
		{
		...
		subscribe(Lang::CurrentCloudManager().firstLanguageSuggestion(), [this]
		{
			createLanguageLink();
		});
		createLanguageLink();
	...
	void Widget::historyMove(Direction direction)
	{
		...
		if (wasStep->hasCover() != getStep()->hasCover())
		{
			...
			_coverShownAnimation.start([this]
			{
				updateControlsGeometry();
			}, 0., 1., st::introCoverDuration, wasStep->hasCover() ? anim::linear : anim::easeOutCirc);
		}
	...
	void Widget::showResetButton()
	{
		if (!_resetAccount)
		{
			...
			_resetAccount->entity()->setClickedCallback([this]
			{
				resetAccount();
			});
			updateControlsGeometry();
