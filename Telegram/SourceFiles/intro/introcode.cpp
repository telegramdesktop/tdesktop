...
namespace Intro
{
		...
		, _checkRequest(this)
	{
		...
		setTitleText([text = App::formatPhone(getData()->phone)]
		{
			return text;
		});
		updateDescText();
	...
	void CodeWidget::codeSubmitDone(const MTPauth_Authorization& result)
	{
		...
		if (d.vuser.type() != mtpc_user || !d.vuser.c_user().is_self())
		{
			// wtf?
	...
	bool CodeWidget::codeSubmitFail(const RPCError& error)
	{
		...
		else if (err == qstr("PHONE_NUMBER_UNOCCUPIED"))
		{
			// success, need to signUp
