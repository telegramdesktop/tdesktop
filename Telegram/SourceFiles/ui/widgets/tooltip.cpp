...
namespace Ui
{
		...
		RpWidget(nullptr)
	{
		...
		_showTimer.setCallback([=]
		{
			performShow();
		});
		_hideByLeaveTimer.setCallback([=]
		{
			Hide();
		});
	...
	void Tooltip::Hide()
	{
		if (auto instance = TooltipInstance)
		{
			...
			instance->hide();
			InvokeQueued(instance, [instance]
			{
	...
	void ImportantTooltip::toggleAnimated(bool visible)
	{
		...
		if (_visible != visible)
		{
			...
			hideChildren();
			_visibleAnimation.start([this]
			{
				animationCallback();
			}, _visible ? 0. : 1., _visible ? 1. : 0., _st.duration, anim::easeOutCirc);
		}
	}
