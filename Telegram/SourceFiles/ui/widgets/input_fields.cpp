...
namespace Ui
{
	namespace
	{
		...
		{
		public:
			TagAccumulator(TextWithTags::Tags& tags) :
				_tags(tags)
			{
			}
	...
	{
	public:
		Inner(not_null<InputField*> parent) :
			QTextEdit(parent)
		{
		}
		...
		, _placeholderFactory(std::move(placeholderFactory))
	{
		...
		subscribe(Lang::Current().updated(), [=]
		{
			refreshPlaceholder();
		});
		refreshPlaceholder();
	...
	void InputField::setFocused(bool focused)
	{
		...
		{
			_focused = focused;
			_a_focused.start([this]
			{
				update();
			}, _focused ? 0. : 1., _focused ? 1. : 0., _st.duration);
			startPlaceholderAnimation();
			startBorderAnimation();
		}
	}
	...
	void InputField::setErrorShown(bool error)
	{
		if (_error != error)
		{
			...
			{
				update();
			}, _error ? 0. : 1., _error ? 1. : 0., _st.duration);
			startBorderAnimation();
		}
	}
	...
	void MaskedInputField::startPlaceholderAnimation()
	{
		...
		{
			_placeholderShifted = placeholderShifted;
			_a_placeholderShifted.start([this]
			{
				update();
			}, _placeholderShifted ? 0. : 1., _placeholderShifted ? 1. : 0., _st.duration);
		}
	}
	...
	void MaskedInputField::keyPressEvent(QKeyEvent* e)
	{
		...
		if (wasText == newText && wasCursor == newCursor)
		{
			// call correct manually
	...
	PasswordInput::PasswordInput(QWidget* parent, const style::InputField& st, Fn<QString()> placeholderFactory, const QString& val) :
	...
	HexInput::HexInput(QWidget* parent, const style::InputField& st, Fn<QString()> placeholderFactory, const QString& val) :
		MaskedInputField(parent, st, std::move(placeholderFactory), val)
	{
	...
	UsernameInput::UsernameInput(QWidget* parent, const style::InputField& st, Fn<QString()> placeholderFactory, const QString& val, bool isLink) :
		MaskedInputField(parent, st, std::move(placeholderFactory), val)
	{
	...
	PhoneInput::PhoneInput(QWidget* parent, const style::InputField& st, Fn<QString()> placeholderFactory, const QString& val) :
		MaskedInputField(parent, st, std::move(placeholderFactory), val)
	{
		QString phone(val);
		if (phone.isEmpty())
