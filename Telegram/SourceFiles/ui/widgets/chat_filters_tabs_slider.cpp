...
QAccessible::State ChatsFiltersTabs::accessibilityChildState(int index) const {
	auto state = Ui::SettingsSlider::accessibilityChildState(index);
	// A locked folder can be browsed, read and pressed - that opens the
	// premium promo - but never becomes the active one, so a selection
	// request must not be accepted for it.
	if (_lockedFrom > 0 && index >= _lockedFrom) {
