...
namespace Ui
{
	...
	void ScrollBar::setOver(bool over)
	{
		if (_over != over)
		{
			...
			if (wasOver != nowOver)
			{
				...
				{
					update();
				}, nowOver ? 0. : 1., nowOver ? 1. : 0., _st->duration);
			}
			if (nowOver && _hiding)
			{
				_hiding = false;
	...
	void ScrollBar::setOverBar(bool overbar)
	{
		if (_overbar != overbar)
		{
			...
			{
				_a_barOver.start([this]
				{
					update();
				}, nowBarOver ? 0. : 1., nowBarOver ? 1. : 0., _st->duration);
			}
		}
	}
		...
		case QEvent::TouchEnd:
			{
				...
				{
					// one short tap -- like left mouse click, one long tap -- like right mouse click
