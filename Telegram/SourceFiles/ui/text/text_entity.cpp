...
namespace TextUtilities
{
	...
	bool checkTagStartInCommand(const QChar* start, int32 len, int32 tagStart, int32& commandOffset, bool& commandIsLink, bool& inLink)
	{
		...
		while (commandOffset < len && tagStart > commandOffset)
		{
			// skip commands, evaluating are we in link or not
				...
				case mtpc_messageEntityUrl:
					{
						...
						result.push_back({EntityType::Url, d.voffset.v, d.vlength.v});
					}
					break;
				...
				case mtpc_messageEntityEmail:
					{
						...
						result.push_back({EntityType::Email, d.voffset.v, d.vlength.v});
					}
					break;
				case mtpc_messageEntityHashtag:
					{
						auto& d = entity.c_messageEntityHashtag();
						result.push_back({EntityType::Hashtag, d.voffset.v, d.vlength.v});
					}
					break;
				case mtpc_messageEntityCashtag:
					{
						auto& d = entity.c_messageEntityCashtag();
						result.push_back({EntityType::Cashtag, d.voffset.v, d.vlength.v});
					}
					break;
				case mtpc_messageEntityPhone:
					break; // Skipping phones.
				case mtpc_messageEntityMention:
					{
						auto& d = entity.c_messageEntityMention();
						result.push_back({EntityType::Mention, d.voffset.v, d.vlength.v});
					}
					break;
				...
				case mtpc_messageEntityBotCommand:
					{
						...
						result.push_back({EntityType::BotCommand, d.voffset.v, d.vlength.v});
					}
					break;
				case mtpc_messageEntityBold:
					{
						auto& d = entity.c_messageEntityBold();
						result.push_back({EntityType::Bold, d.voffset.v, d.vlength.v});
					}
					break;
				case mtpc_messageEntityItalic:
					{
						auto& d = entity.c_messageEntityItalic();
						result.push_back({EntityType::Italic, d.voffset.v, d.vlength.v});
					}
					break;
				case mtpc_messageEntityCode:
					{
						auto& d = entity.c_messageEntityCode();
						result.push_back({EntityType::Code, d.voffset.v, d.vlength.v});
					}
					break;
				case mtpc_messageEntityPre:
					{
						auto& d = entity.c_messageEntityPre();
						result.push_back({EntityType::Pre, d.voffset.v, d.vlength.v, Clean(qs(d.vlanguage))});
					}
					break;
					// #TODO entities
				}
	...
	MTPVector<MTPMessageEntity> EntitiesToMTP(const EntitiesInText& entities, ConvertOption option)
	{
		...
		for_const(auto &entity, entities)
		{
			...
			case EntityType::Code:
				...
				break; // #TODO entities
			case EntityType::Pre:
				v.push_back(MTP_messageEntityPre(offset, length, MTP_string(entity.data())));
				break;
			}
		}
