...
{
public:
	ITextBlock(const style::font& font, const QString& str, uint16 from, uint16 length, uchar flags, uint16 lnkIndex) :
		_from(from), _flags((flags & 0xFF) | ((lnkIndex & 0xFFFF) << 12))
	{
	}
...
{
public:
	NewlineBlock(const style::font& font, const QString& str, uint16 from, uint16 length, uchar flags, uint16 lnkIndex) :
		ITextBlock(font, str, from, length, flags, lnkIndex), _nextDir(Qt::LayoutDirectionAuto)
	{
