...
void CountrySelectBox::Inner::focusInEvent(QFocusEvent *e) {
	// Pick the row before the base class runs: where the platform resolves
	// the focus event Qt raises for the widget to focusChild() that
	// announces the row, and the helper covers the platform that does
	...
	if (_selected >= 0) {
		accessibilityChildFocusedByEntry(_selected);
	}
}
