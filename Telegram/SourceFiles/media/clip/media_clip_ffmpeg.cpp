	...
	namespace Clip
	{
		namespace internal
		{
			...
			FFMpegReaderImplementation::Rotation FFMpegReaderImplementation::rotationFromDegrees(int degrees) const
			{
				...
				{
				case 90:
					return Rotation::Degrees90;
				case 180:
					return Rotation::Degrees180;
				case 270:
					return Rotation::Degrees270;
				}
			...
			int64_t FFMpegReaderImplementation::_seek(void* opaque, int64_t offset, int whence)
			{
				...
				{
				case SEEK_SET:
					return l->_device->seek(offset) ? l->_device->pos() : -1;
				case SEEK_CUR:
					return l->_device->seek(l->_device->pos() + offset) ? l->_device->pos() : -1;
				case SEEK_END:
					return l->_device->seek(l->_device->size() + offset) ? l->_device->pos() : -1;
				case AVSEEK_SIZE:
					{
