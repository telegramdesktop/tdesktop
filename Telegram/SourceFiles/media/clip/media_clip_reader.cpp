...
namespace Media
{
	namespace Clip
	{
		...
		Reader::Frame* Reader::frameToShow(int32* index) const
		{
			// 0 means not ready
		...
		Reader::Frame* Reader::frameToWrite(int32* index) const
		{
			// 0 means not ready
		...
		public:
			...
			ProcessResult start(crl::time ms)
			{
				...
				if (frame()->original.isNull())
				{
					...
					else if (readResult != internal::ReaderImplementation::ReadResult::Success)
					{
			...
			ProcessResult process(crl::time ms)
			{
				// -1 - do nothing, 0 - update, 1 - reinit
				if (_state == State::Error)
				{
		...
		bool Manager::handleProcessResult(ReaderPrivate* reader, ProcessResult result, crl::time ms)
		{
			...
			if (!reader->_autoPausedGif && reader->_mode == Reader::Mode::Gif && result == ProcessResult::Repaint)
			{
				...
				if (reader->_frames[ishowing].when > 0 && showing->displayed.loadAcquire() <= 0)
				{
					// current frame was not shown
