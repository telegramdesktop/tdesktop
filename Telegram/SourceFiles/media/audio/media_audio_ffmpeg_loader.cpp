...
namespace Media
{
	...
	int64_t AbstractFFMpegLoader::_seek_data(void* opaque, int64_t offset, int whence)
	{
		...
		case SEEK_SET:
			...
			break;
		case SEEK_CUR:
			newPos = l->_dataPos + offset;
			break;
		case SEEK_END:
			newPos = l->_data.size() + offset;
			break;
		case AVSEEK_SIZE:
			{
	...
	int64_t AbstractFFMpegLoader::_seek_bytes(void* opaque, int64_t offset, int whence)
	{
		...
		case SEEK_CUR:
			...
			break;
		case SEEK_END:
			newPos = static_cast<int>(l->_bytes.size()) + offset;
			break;
		case AVSEEK_SIZE:
			{
				// Special whence for determining filesize without any seek.
	...
	int64_t AbstractFFMpegLoader::_seek_file(void* opaque, int64_t offset, int whence)
	{
		...
		{
		case SEEK_SET:
			return l->_f.seek(offset) ? l->_f.pos() : -1;
		case SEEK_CUR:
			return l->_f.seek(l->_f.pos() + offset) ? l->_f.pos() : -1;
		case SEEK_END:
			return l->_f.seek(l->_f.size() + offset) ? l->_f.pos() : -1;
		case AVSEEK_SIZE:
			{
				// Special whence for determining filesize without any seek.
				return l->_f.size();
