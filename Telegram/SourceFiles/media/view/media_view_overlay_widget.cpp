...
namespace Media
{
	...
	if (image.width() > kMaxDisplayImageSize
		...
		{
			SharedMedia(SharedMediaKey key) :
				key(key)
			{
			}
			...
			, _dropdownShowTimer(this)
		{
			...
			_saveMsgText.setLink(1, std::make_shared<LambdaClickHandler>([this]
			{
				showSaveMsgFile();
			}));
	...
	setWindowFlags(Qt::FramelessWindowHint | Qt::MaximizeUsingFullscreenGeometryHint);
			...
			_dropdown->setHiddenCallback([this]
			{
				dropdownHidden();
			});
			...
			crl::async([=, data = std::move(current)]() mutable
			{
				...
				crl::on_main(weak, [=, result = std::move(preview)]() mutable
				{
					...
					if (_themePreview)
					{
						...
						_themeCancel->show();
						_themeCancel->setClickedCallback([this]
						{
							close();
						});
						updateControls();
