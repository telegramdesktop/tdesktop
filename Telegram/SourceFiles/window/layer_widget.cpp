...
namespace Window
{
	...
	void LayerStackWidget::BackgroundWidget::setMainMenuShown(bool shown)
	{
		...
		if (_mainMenuShown != shown)
		{
			...
			_a_mainMenuShown.start([this]
			{
				animationCallback();
			}, _mainMenuShown ? 0. : 1., _mainMenuShown ? 1. : 0., st::boxDuration, anim::easeOutCirc);
		}
	...
	void LayerStackWidget::BackgroundWidget::setSpecialLayerShown(bool shown)
	{
		...
		if (_specialLayerShown != shown)
		{
			...
			_a_specialLayerShown.start([this]
			{
				animationCallback();
			}, _specialLayerShown ? 0. : 1., _specialLayerShown ? 1. : 0., st::boxDuration);
		}
		checkWasShown(wasShown);
	}
	...
	void LayerStackWidget::BackgroundWidget::setLayerShown(bool shown)
	{
		...
		{
			_layerShown = shown;
			_a_layerShown.start([this]
			{
				animationCallback();
			}, _layerShown ? 0. : 1., _layerShown ? 1. : 0., st::boxDuration);
		}
		checkWasShown(wasShown);
	}
	...
	void LayerStackWidget::BackgroundWidget::checkWasShown(bool wasShown)
	{
		if (isShown() != wasShown)
		{
			...
			{
				animationCallback();
			}, wasShown ? 1. : 0., wasShown ? 0. : 1., st::boxDuration, anim::easeOutCirc);
		}
	}
		...
		, _background(this)
	{
		...
		hide();
		_background->setDoneCallback([this]
		{
			animationDone();
		});
	}
	...
	void LayerStackWidget::initChildLayer(LayerWidget* layer)
	{
		...
		layer->setClosedCallback([=]
		{
			closeLayer(layer);
		});
		layer->setResizedCallback([=]
		{
			updateLayerBoxes();
		});
