...
namespace Window
{
	namespace Theme
	{
		...
		ThemeExportBox::ThemeExportBox(QWidget*, const QByteArray& paletteContent, const QImage& background, const QByteArray& backgroundContent, bool tileBackground) :
			BoxContent()
			...
			, _tileBackground(this, lang(lng_settings_bg_tile), tileBackground, st::defaultBoxCheckbox)
		{
			...
			_chooseFromFile->setClickedCallback([this]
			{
		...
		void ThemeExportBox::prepare()
		{
			...
			addButton(langFactory(lng_theme_editor_export), [this]
			{
				exportTheme();
			});
			...
			, _export(this, lang(lng_theme_editor_export_button).toUpper(), st::dialogsUpdateButton)
		{
			...
			{
				App::CallDelayed(2 * st::boxDuration, this, [this]
				{
			...
			_select->setSubmittedCallback([this](Qt::KeyboardModifiers)
			{
				_inner->chooseRow();
			});
