		...
		namespace Default
		{
			...
			void Manager::settingsChanged(ChangeType change)
			{
				...
				else if (change == ChangeType::DemoIsShown)
				{
					...
					_demoMasterOpacity.start([this]
					{
						demoMasterOpacityCallback();
					}, demoIsShown ? 1. : 0., demoIsShown ? 0. : 1., st::notifyFastAnim);
				}
			}
			...
			namespace internal
			{
					...
					, _reply(this, langFactory(lng_notification_reply), st::defaultBoxButton)
				{
					...
					connect(&_hideTimer, &QTimer::timeout, [=]
					{
						startHiding();
					});
				...
				void Notification::toggleActionButtons(bool visible)
				{
					if (_actionsVisible != visible)
					{
						...
						a_actionsOpacity.start([this]
						{
							actionsOpacityCallback();
						}, _actionsVisible ? 0. : 1., _actionsVisible ? 1. : 0., st::notifyActionsDuration);
						_reply->clearState();
						_reply->hide();
					}
				}
				...
				void Notification::showReplyField()
				{
					...
					connect(_replyArea, &Ui::InputField::resized, [=]
					{
						replyResized();
					});
					connect(_replyArea, &Ui::InputField::submitted, [=]
					{
						sendReply();
					});
					connect(_replyArea, &Ui::InputField::cancelled, [=]
					{
						replyCancel();
					});
					...
					_replySend->show();
					_replySend->setClickedCallback([this]
					{
						sendReply();
					});
