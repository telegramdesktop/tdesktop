...
private:
	...
	// The sticker the keyboard is on, to be found again after a rebuild
	// of the cells: by its document and the set it is in, or by its
	// place in the list when it is gone.
	struct KeyboardItem {
		uint64 setId = 0;
	...
	// The list a screen reader sees: every sticker, set by set - of a
	// featured set the first row only, as shown.
	[[nodiscard]] int renderedCount(const SectionInfo &info) const;
	...
	void returnFocus();
	void focusFromSearch(const OverSticker &over);
