...
, _forceFirstFrame(descriptor.forceFirstFrame) {
	...
	Ui::ScreenReaderModeActiveValue(
		...
		// The reader is gone: the strip is not focusable any more, and a
		// focus it kept is let go of.
		if (hasFocus()) {
			clearFocus();
		}
		_keyboardSelected = -1;
...
quintptr StickersListFooter::accessibilityChildIdentity(int index) const {
	// The place in the list names the item, within the generation of the
	// icons it is in; the tag bit keeps it non-zero.
...
int StickersListFooter::accessibilityChildIndexByIdentity(
	...
	return (index < accessibilityChildCount()) ? index : -1;
}
...
void StickersListFooter::focusInEvent(QFocusEvent *e) {
	...
	// Real Tab traversal lands on the section shown; an accessibility
	// SetFocus (plain setFocus(), OtherFocusReason) has just picked the
	// item to land on and keeps it.
	const auto count = accessibilityChildCount();
	if (!count) {
		return;
	}
	...
	const auto walked = (_keyboardSelected >= 0 && _keyboardSelected < count);
...
void StickersListFooter::refreshIcons(
	...
	// The icons were rebuilt: an action queued against the old ones
	// cancels, and the keyboard follows its item by set, if still there.
	const auto walked = accessibleChild(_keyboardSelected);
	const auto walkedSetId = (walked
		&& !walked->settings
		&& walked->icon.index < _icons.size())
		? _icons[walked->icon.index].setId
		: uint64(0);
	_icons = std::move(icons);
	++_generation;
	_keyboardSelected = -1;
	if (walked && walked->settings) {
		_keyboardSelected = accessibleIndex(OverState(SpecialOver::Settings));
	} else if (walkedSetId) {
		const auto i = ranges::find(_icons, walkedSetId, &StickerIcon::setId);
		if (i != end(_icons)) {
			_keyboardSelected = accessibleIndex(IconId{
				.index = int(i - begin(_icons)),
				.subindex = walked->icon.subindex,
			});
		}
	}
