...
void AddContactBox::prepare()
{
	...
	connect(_first, &Ui::InputField::submitted, [=]
	{
		submit();
	});
	connect(_last, &Ui::InputField::submitted, [=]
	{
		submit();
	});
	connect(_phone, &Ui::PhoneInput::submitted, [=]
	{
		submit();
	});
...
void AddContactBox::updateButtons()
{
	...
	if (_retrying)
	{
		addButton(langFactory(lng_try_other_contact), [this]
		{
			retry();
		});
	}
	...
	{
		addButton(langFactory(_user ? lng_settings_save : lng_add_contact), [this]
		{
			save();
		});
...
void GroupInfoBox::prepare()
{
	...
	connect(_title, &Ui::InputField::submitted, [=]
	{
		submitName();
	});

	addButton(langFactory((_type != Type::Group) ? lng_create_group_create : lng_create_group_next), [this]
	{
		submit();
	});
	addButton(langFactory(lng_cancel), [this]
...
void SetupChannelBox::prepare()
{
	...
	addButton(langFactory(_existing ? lng_cancel : lng_create_group_skip), [=]
	{
		closeBox();
	});

	connect(_link, &Ui::MaskedInputField::changed, [=]
	{
		handleChange();
	});
