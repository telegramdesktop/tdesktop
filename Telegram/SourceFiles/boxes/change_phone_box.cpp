...
void ChangePhoneBox::EnterPhone::prepare()
{
	...
	addButton(langFactory(lng_change_phone_new_submit), [this]
	{
		submit();
	});
...
void ChangePhoneBox::EnterPhone::sendPhoneDone(const QString& phoneNumber, const MTPauth_SentCode& result)
{
	...
	case mtpc_auth_sentCodeTypeSms:
		codeLength = data.vtype.c_auth_sentCodeTypeSms().vlength.v;
		break;
	case mtpc_auth_sentCodeTypeCall:
		codeLength = data.vtype.c_auth_sentCodeTypeCall().vlength.v;
		break;
...
void ChangePhoneBox::EnterCode::prepare()
{
	...
	_code->setAutoSubmit(_codeLength, [=]
	{
		submit();
	});
	...
	addButton(langFactory(lng_change_phone_new_submit), [=]
	{
		submit();
	});
