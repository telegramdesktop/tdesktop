...
void SendFilesBox::AlbumPreview::updateSuggestedDrag(QPoint position)
{
	...
	if (_suggestedThumb != _draggedThumb)
	{
		...
		{
			const auto next = _thumbs[_order[suggestedIndex + 1]].get();
			next->suggestMove(-delta, [=]
			{
				update();
			});
		}
		_suggestedThumb->suggestMove(delta, [=]
		{
			update();
		});
	}
...
{
	_send = addButton(langFactory(lng_send_button), [=]
	{
		send();
	});
...
void SendFilesBox::setupCaption()
{
	...
	connect(_caption, &Ui::InputField::cancelled, [=]
	{
		closeBox();
	});
