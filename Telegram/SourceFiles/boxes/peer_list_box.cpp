...
void PeerListBox::createMultiSelect()
{
	...
	_select->heightValue(
		...
		lifetime());
	_select->entity()->setSubmittedCallback([this](Qt::KeyboardModifiers)
	{
		content()->submitted();
	});
	_select->entity()->setQueryChangedCallback([this](const QString& query)
	{
	...
	, _rowHeight(_st.item.height)
{
	...
	_repaintByStatus.setCallback([this]
	{
		update();
	});
}
