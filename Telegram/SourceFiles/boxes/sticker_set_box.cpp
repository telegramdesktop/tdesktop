...
void StickerSetBox::updateButtons()
{
	...
	if (_inner->loaded())
	{
		...
		{
			addButton(langFactory(lng_stickers_add_pack), [=]
			{
				addStickers();
			});
		...
		{
			addButton(langFactory(lng_about_done), [=]
			{
				closeBox();
			});
		}
		...
		{
			addButton(langFactory(lng_stickers_share_pack), [=]
			{
				shareStickers();
			});
			addButton(langFactory(lng_cancel), [=]
	...
	})
{
	...
	case mtpc_inputStickerSetShortName:
		_setShortName = qs(set.c_inputStickerSetShortName().vshort_name);
		break;
	}
...
Fn<TextWithEntities()> StickerSetBox::Inner::title() const
{
	...
	if (_loaded)
	{
		if (_pack.isEmpty())
		{
			return []
			{
				return TextWithEntities{lang(lng_attach_failed), EntitiesInText()};
			};
		}
		else
		{
	...
	else
	{
		return []
		{
			return TextWithEntities{lang(lng_contacts_loading), EntitiesInText()};
		};
	}
	return [text]
	{
