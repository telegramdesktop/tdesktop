...
{
	subscribe(_installed.widget()->scrollToY, [this](int y)
	{
		onScrollToY(y);
	});
}
...
void StickersBox::prepare()
{
	...
	if (_featured.widget())
	{
		_featured.widget()->setInstallSetCallback([this](uint64 setId)
		{
			installSet(setId);
		});
	}
	if (_archived.widget())
	{
		_archived.widget()->setInstallSetCallback([this](uint64 setId)
		{
			installSet(setId);
		});
		_archived.widget()->setLoadMoreCallback([this]
		{
			loadMoreArchived();
		});
	}
	if (_attached.widget())
	{
		_attached.widget()->setInstallSetCallback([this](uint64 setId)
		{
			installSet(setId);
		});
		_attached.widget()->setLoadMoreCallback([this]
		{
	...
	else
	{
		...
		addButton(langFactory(close ? lng_close : lng_about_done), [this]
		{
			closeBox();
		});
	}
...
void StickersBox::switchTab()
{
	...
	_slideAnimation->start(slideLeft, [this]
	{
		update();
	}, st::slideDuration);
...
StickersBox::Inner::Inner(QWidget* parent, not_null<ChannelData*> megagroup) :
	...
	, _megagroupSetField(this, st::groupStickersField, []
	{
		return qsl("stickerset");
	}, QString(), true)
