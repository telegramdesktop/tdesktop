...
MTPInputPrivacyKey ApiWrap::Privacy::Input(Key key)
{
	switch (key)
	{
	case Privacy::Key::Calls:
		return MTP_inputPrivacyKeyPhoneCall();
	case Privacy::Key::Invites:
		return MTP_inputPrivacyKeyChatInvite();
	case Privacy::Key::PhoneNumber:
	...
	mtpTypeId type)
{
	...
	case mtpc_inputPrivacyKeyPhoneNumber:
...
ApiWrap::ApiWrap(not_null<AuthSession*> session):
	...
	, _proxyPromotionTimer([=]
	{
		refreshProxyPromotion();
	})
	, _updateNotifySettingsTimer([=]
	{
		sendNotifySettingsUpdates();
	})
{
	crl::on_main([=]
	{
...
void ApiWrap::requestNotifySettings(const MTPInputNotifyPeer& peer)
{
	const auto key = [&]
	{
		...
		case mtpc_inputNotifyUsers:
			return peerFromUser(0);
		case mtpc_inputNotifyChats:
			return peerFromChat(0);
		case mtpc_inputNotifyBroadcasts:
			return peerFromChannel(0);
	...
	int currentOnlineTill)
{
	...
	case mtpc_userStatusOffline:
		return status.c_userStatusOffline().vwas_online.v;
	case mtpc_userStatusOnline:
		return status.c_userStatusOnline().vexpires.v;
	}
...
void ApiWrap::requestFeaturedStickers(TimeId now)
{
	...
	)).done([=](const MTPmessages_FeaturedStickers& result)
	{
		...
		switch (result.type())
		{
		case mtpc_messages_featuredStickersNotModified:
			return;
	...
	case mtpc_updateNewMessage:
		{
			...
			auto needToAdd = true;
			if (d.vmessage.type() == mtpc_message)
			{
				// index forwarded messages to links _overview
				if (_session->data().checkEntitiesAndViewsUpdate(d.vmessage.c_message()))
				{
					// already in blocks
