...
using VersionChar = wchar_t;
	...
	const int32 hSigLen = 128, hShaLen = 20, hPropsLen = 0, hOriginalSizeLen = sizeof(int32), hSize = hSigLen + hShaLen + hOriginalSizeLen; // header
			...
			if (RSA_verify(NID_sha1, (const uchar*)(compressed.constData() + hSigLen), hShaLen, (const uchar*)(compressed.constData()), hSigLen, pbKey) != 1)
			{
				// verify signature
				RSA_free(pbKey);
	...
	if (res != LZMA_OK && res != LZMA_STREAM_END) {
		...
		HttpChecker::HttpChecker(bool testing) :
			Checker(testing)
		{
		}
	...
	QFileInfo updater(cWorkingDir() + qsl("tupdates/temp/Updater"));
		...
		if (CopyFile(updater.absoluteFilePath().toStdWString().c_str(), curUpdater.toStdWString().c_str(), FALSE) == FALSE)
		{
			...
			{
				// we are in write-protected dir, like Program Files
	...
	QString countAlphaVersionSignature(uint64 version)
	{
		...
		if (RSA_sign(NID_sha1, (const uchar*)(sha1Buffer), shaSize, (uchar*)(signature.data()), &siglen, prKey) != 1)
		{
			// count signature
