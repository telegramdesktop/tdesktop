...
namespace Info
{
	...
	void TopBar::createSelectionControls()
	{
		...
		_forward->entity()->addClickHandler([this]
		{
			performForward();
		});
		...
		registerToggleControlCallback(
			...
			[this]
			{
				return selectionMode() && _canDelete;
			});
		_delete->setDuration(st::infoTopBarDuration);
		_delete->entity()->addClickHandler([this]
		{
			performDelete();
		});
	...
	Ui::StringWithNumbers TopBar::generateSelectedText() const
	{
		...
		auto phrase = [&]
		{
			...
			{
			case Type::Photo:
				return lng_media_selected_photo__generic<Data>;
			case Type::Video:
				return lng_media_selected_video__generic<Data>;
			case Type::File:
				return lng_media_selected_file__generic<Data>;
			case Type::MusicFile:
				return lng_media_selected_song__generic<Data>;
			case Type::Link:
				return lng_media_selected_link__generic<Data>;
			case Type::RoundVoiceFile:
				return lng_media_selected_audio__generic<Data>;
			}
