...
"""Stage IWYU's edits and commit them as `github-actions[bot]` on a
fresh branch (default: `Telegram/SourceFiles/` only, tracked changes
only). Optional --push."""
...
def main() -> int:
    ...
    p.add_argument("--include-untracked", action="store_true",
                   help="Stage untracked files in scope (default: tracked only).")
