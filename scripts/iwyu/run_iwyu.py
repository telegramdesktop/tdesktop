...
"""
Drive the include-what-you-use cleanup pipeline end-to-end.

Stages, each individually skippable:

  1. configure  — `cmake -B <build-dir> -DDESKTOP_APP_USE_IWYU=ON
                  -DCMAKE_EXPORT_COMPILE_COMMANDS=ON ...`
  2. build      — `cmake --build <build-dir> --target <target>` so codegen
                  products exist and compile_commands.json is complete.
  3. analyze    — runs `iwyu_tool.py -p <build-dir>` against the project
                  source list, writing raw IWYU output to a file.
  4. filter     — drops blocks for submodule paths (read live from
                  .gitmodules) and, when requested, current-platform files.
  5. apply      — feeds the filtered output through `fix_includes.py`,
                  bounded with `--only_re=Telegram/SourceFiles/`.
  6. commit     — delegates to iwyu_commit.py to make a branch + commit
                  authored by the GitHub Action identity, optionally push.

Skip stages individually with --skip-* flags. Default flow is the local
"see what would change" pass (no commit).
"""
...
def _run(cmd: list[str], *, cwd: Path | None = None,
         ...
         check: bool = True) -> int:
    """Run `cmd`. Optionally tee stdout to a file. Echo command first."""
...
def build(build_dir: Path, target: str, jobs: int) -> None:
    ...
    # ninja -k 0 keeps going past IWYU's non-zero exits when the per-target
    # property is on. iwyu_tool.py mode does not need this, but it does not
    # hurt either.
...
def list_in_scope_files(build_dir: Path, repo_root: Path,
    ...
    """Read compile_commands.json and return absolute paths under project_subdir."""
...
def _detect_resource_dir(build_dir: Path) -> str | None:
    """Return the actual compiler's `-print-resource-dir`, or None.

    IWYU bundles its own clang. On macOS that means it cannot find Apple
    Clang's libc++/libc headers (climits, type_traits, ...). Pinning the
    resource directory to the configured CMAKE_CXX_COMPILER closes the gap.
    """
...
def _detect_macos_sysroot() -> str | None:
    """`xcrun --show-sdk-path`. Apple Clang knows its SDK implicitly; IWYU
    does not, so without this it cannot find <cmath>, <type_traits>, etc."""
...
def _strip_pch_from_command(tokens: list[str]) -> list[str]:
    """Remove `-Xclang -include-pch <path>` triples from a compile command.

    IWYU rejects compile commands that load a precompiled binary, but the
    cmake_pch.hxx header itself is still pulled in via a separate
    `-Xclang -include <header>` triple — which we keep, so all PCH-provided
    declarations remain visible to IWYU.
    """
    out: list[str] = []
    i = 0
    while i < len(tokens):
        ...
        if (t == "-Xclang"
            ...
            i += 4  # skip -Xclang -include-pch -Xclang <path>
            continue
...
def _make_iwyu_compile_db(build_dir: Path) -> Path:
    """Return a directory holding a PCH-stripped copy of compile_commands.json."""
...
def apply_fixes(repo_root: Path, filtered: Path, only_under: str,
    ...
    """Apply IWYU edits. Two backends:

    * `fix_includes.py` (default) — full IWYU sweep: removes, additions,
      grouping, sorting, comment updates.
    * `iwyu_remove_only.py` (when `removes_only=True`) — deletes the
      lines listed in `should remove` blocks and nothing else. With
      `verify_db` set, every edited file is then run through
      -fsyntax-only and rolled back on failure; with `skip_headers`,
      .h removal blocks are dropped before applying.
    """
    if removes_only:
...
def _split_argv_at_double_dash(argv: list[str]) -> tuple[list[str], list[str]]:
    """Split sys.argv tail at the first standalone '--' token.

    Everything before stays subject to argparse; everything after is forwarded
    verbatim to cmake configure, so callers can write
        run_iwyu.py --skip-platform=auto -- -DFOO=BAR -DBAZ=QUX
    instead of repeating --cmake-arg= for every flag.
    """
    if "--" not in argv:
        return argv, []
    idx = argv.index("--")
...
def main() -> int:
    ...
    p.add_argument("--cmake-arg", action="append", default=[],
                   help="Extra arg passed to cmake configure. May be "
                        "repeated. Equivalent — and usually shorter — is to "
                        "list every cmake arg after a literal '--' at the "
                        "end of the command line; both forms are merged.")
    ...
    p.add_argument("--no-adds", action="store_true",
                   help="Cleanup-only mode: strip every 'should add these "
                        "lines:' section before applying. fix_includes.py "
                        "then only deletes unused includes, never adds new "
                        "explicit ones for transitively-available symbols.")
    p.add_argument("--skip-headers", action="store_true",
                   help="With --no-adds, drop every removal targeting a "
                        "header file. Header edits propagate to all "
                        "includers and are unsafe to apply unverified.")
    p.add_argument("--verify", action="store_true",
                   help="With --no-adds, after applying each removal run "
                        "-fsyntax-only against the file using its entry "
                        "from compile_commands.json; revert files whose "
                        "syntax check fails. Replaces the iterative cmake "
                        "build/revert loop.")
