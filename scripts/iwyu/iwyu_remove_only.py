...
"""Apply only the `should remove` portion of IWYU output, no reordering
or additions. With `--verify`, every edited TU is syntax-checked against
its compile_commands.json entry and reverted on failure; modified
headers are tested via their dependent TUs through `ninja -t deps`."""
...
def apply_removes(file_path: Path, directives: list[str],
    ...
    """Returns (matched, missing, original_text) — pass `original_text`
    to `restore_file` to roll back the edit."""
...
def build_header_dep_index(build_dir: Path,
    ...
    """Reverse-map header → set of TUs that include it via `ninja -t deps`.
    `tu_filter` keeps the anchor pinned to actual sources, skipping the
    cmake_pch.hxx.cxx wrapper which `ninja -t deps` lists first."""
    try:
        result = subprocess.run(
            ["ninja", "-t", "deps"],
...
def main() -> int:
    ...
    # TUs first (against pristine headers), then headers one at a time
    # so a bad header does not contaminate the next header's check.
